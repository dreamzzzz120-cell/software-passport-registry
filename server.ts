...
app.get('/health', (_req, res) => {
  res.status(200).json({ status: 'ok', service: 'spr-app', uptimeSeconds: Math.floor((Date.now() - startedAt) / 1000) });
});
...
app.use((err: any, req: Request, res: Response, next: NextFunction) => {
  ...
  const requestId = res.locals.requestId || `req_${cryptoRandomId()}`;
...
export async function startServer() {
  ...
  server = app.listen(config.port, host, () => {
    console.info(`[SPR] listening on http://${host}:${config.port}`);
  });
  return server;
}
...
process.on('uncaughtException', (error) => {
  console.error('[SPR] Uncaught exception:', error);
  void shutdown('uncaughtException');
});

if (process.env.SPR_SKIP_AUTOSTART !== 'true') {
  void startServer().catch((error) => {
    console.error('[SPR] Startup failed:', error);
    process.exit(1);
  });
}
