...
if (config.sentry.dsn) {
  Sentry.init({ dsn: config.sentry.dsn, environment: config.nodeEnv, tracesSampleRate: config.isProduction ? 0.1 : 1.0 });
}
...
const corsOrigin = (origin: string | undefined, callback: (error: Error | null, allow?: boolean) => void) => {
  ...
  if (!origin && config.isProduction) return callback(null, true); // same-origin/server-to-server request
...
app.get('/ready', async (_req, res) => {
  ...
  res.status(ready ? 200 : 503).json({ status: ready ? 'ready' : 'not_ready', checks: { database }, uptimeSeconds: Math.floor((Date.now() - startedAt) / 1000) });
});
...
app.get('*', (req, res, next) => {
  ...
  return res.sendFile(path.join(publicDir, 'index.html'), (error) => error ? next(error) : undefined);
});
...
async function shutdown(signal: string) {
  ...
  shuttingDown = true;
  console.info(`[SPR] ${signal} received; shutting down gracefully.`);
  const forceTimer = setTimeout(() => process.exit(1), 15_000);
  forceTimer.unref();
  if (server) await new Promise<void>((resolve) => server!.close(() => resolve()));
  await closeDatabase().catch((error) => console.error('[SPR] Database shutdown error:', error));
  if (config.sentry.dsn) await Sentry.close(2_000).catch(() => undefined);
...
export async function startServer() {
  ...
  server = app.listen(config.port, host, () => console.info(`[SPR] listening on http://${host}:${config.port}`));
  return server;
}

process.once('SIGTERM', () => void shutdown('SIGTERM'));
process.once('SIGINT', () => void shutdown('SIGINT'));
process.on('unhandledRejection', (reason) => console.error('[SPR] Unhandled rejection:', reason));
process.on('uncaughtException', (error) => { console.error('[SPR] Uncaught exception:', error); void shutdown('uncaughtException'); });

if (process.env.SPR_SKIP_AUTOSTART !== 'true') {
  void startServer().catch((error) => { console.error('[SPR] Startup failed:', error); process.exit(1); });
}
