...
export type AuditChainRow = { id: number; action: string; timestamp: string; actor: string; payload: string; previousHash: string; currentHash: string };

function computeHash(action: string, timestamp: string, actor: string, payload: string, previousHash: string) {
  return crypto.createHash('sha256').update(action + timestamp + actor + payload + previousHash).digest('hex');
}

/** Appends one block to the tenant's audit_trail hash chain (see migration 0000). */
export async function appendAuditEntry(db: ScopedDb, params: { tenantId: string; action: string; actor: string; payload: Record<string, unknown> }) {
  const timestamp = new Date().toISOString();
  const payloadJson = JSON.stringify(params.payload ?? {});
  const last = await db.execute(sql`SELECT current_hash AS "currentHash" FROM audit_trail WHERE tenant_id = ${params.tenantId} ORDER BY id DESC LIMIT 1`);
...
/** Recomputes every block's hash in order and checks it against the stored value and chain linkage. */
export async function verifyAuditChain(db: ScopedDb, tenantId: string) {
  ...
  for (const row of rows) {
    const recomputed = computeHash(row.action, row.timestamp, row.actor, row.payload, expectedPrevious);
    const valid = recomputed === row.currentHash && row.previousHash === expectedPrevious;
