...
// only providers with a genuine multi-customer concept are listed here --
// single-tenant tools (GitLab, Slack, AWS, etc.) have no "customers" to
// discover and are intentionally absent.
...
export async function discoverProviderCustomers(provider: CustomerDiscoveryProvider, credentials: ProviderCredentials): Promise<DiscoveredCustomer[]> {
  switch (provider) {
    case 'connectwise': {
      ...
      const { body } = await safeRequestJson(url, { headers: { Authorization: `Basic ${auth}` } });
    ...
    case 'autotask': {
      ...
      const { body } = await safeRequestJson(url, {
        method: 'POST',
    ...
    case 'ninjaone': {
      ...
      const { body } = await safeRequestJson(url, { headers: bearer(credentials) });
      if (!Array.isArray(body)) throw new Error('PROVIDER_RESPONSE_UNEXPECTED_SHAPE');
    ...
    case 'hudu': {
      ...
      const { body } = await safeRequestJson(url, { headers: { 'x-api-key': credentials.apiKey } });
