...
export interface Client { id: string; name: string; domain: string; industry: string; trustScore: number; riskLevel: RiskLevel; avatarColor: string; subscriptionTier: 'Standard' | 'Enterprise' | 'Premium'; joinedDate: string; teamCount: number; passportCount: number; criticalRisksCount: number; complianceProgress: number; softwareInventory: { passportId: string; name: string; version: string; overallScore: number; riskStatus: 'Safe' | 'Warning' | 'Critical'; lastScanDate: string; }[]; complianceStatus: ComplianceStandard[]; teamMembers: TeamMember[]; activityTimeline: ActivityLog[]; }
