...
export type Finding = { id:string; tenantId:string; passportId:string; clientId:string|null; assetId:string; controlId:string; title:string; severity:Severity; status:FindingStatus; description:string; remediation:string; evidenceIds:string[]; fingerprint:string; policyVersion:string };
...
export async function persistTrustLoop(input:{tenantId:string;passportId:string;clientId:string|null;assetId:string;observations:ControlObservation[];generationReason?:string;actorType?:string;collectorVersionMap?:Record<string,string>}){
  // clientId is intentionally allowed to be null: a passport not owned by any
  // MSP client (e.g. a self-hosted passport for SPR's own software) is a
  // real, supported state -- passports.client_id is nullable for exactly
  // this reason. Fabricating a client id here (this used to happen one level
  // up, in routes/trust-loop.ts) previously violated trust_observations'
  // foreign key to clients() for any client-less passport, failing every
  // provider's collection. See migrations/0027_client_optional_trust_loop.
  if(!input.tenantId||!input.passportId||!input.assetId)throw new Error('TRUST_LOOP_SCOPE_REQUIRED');
