...
// Free Review lets an anonymous visitor run a real scan against a public
// GitHub repository without signing up. It deliberately reuses the exact
// repository_scan / repository_security_scan job pipeline authenticated
// users trigger via POST /api/scans/repository (src/routes/scans.ts) -- the
// same osv-worker.ts / security-scanner-worker.ts pick these jobs up and
// populate real evidence_items / scan_findings rows. No mock data, no
// fabricated pass/fail: a submission that hasn't finished scanning yet is
// reported as still scanning, not silently backfilled with placeholder
// results.
//
// Every submission is scoped to one fixed system tenant rather than a real
// user's tenant, since there is no authenticated identity here. This is not
// a special-cased data path: attachTenantScope() and the worker pipeline
// treat this tenant like any other, the same way public-connect.ts already
// serves unauthenticated passport/report reads against a real tenant with
// no requireAuth in front of it.
...
function hashIp(req: { ip?: string; socket: { remoteAddress?: string } }) {
  ...
  // Keyed with the same secret used for the public share tokens -- this is
  // only ever compared for exact equality (the daily abuse cap), never
  // reversed, so an HMAC is sufficient and avoids persisting a visitor's
  // raw IP address.
...
export function createFreeReviewRouter() {
  ...
  router.post('/free-review/scan', async (req, res, next) => {
    try {
      ...
      if (recentCount >= DAILY_SUBMISSIONS_PER_IP) {
        return res.status(429).json({ error: 'Free Review daily limit reached for this network. Try again tomorrow, or sign up for continuous scanning.' });
      }

      const passportId = id('passport_free');
      // Reuse-or-create one public GitHub connection for the system tenant,
      // mirroring scans.ts's POST /scans/repository (lines 234-236) exactly.
      ...
      if (!existingConnection) {
        await scopedDb.execute(sql`INSERT INTO repository_connections (id,tenant_id,provider,installation_id,label,access_mode,status) VALUES (${connectionId},${FREE_REVIEW_TENANT_ID},'github','public-github','Free Review public GitHub acquisition','public','Active')`);
      }

      // Deliberately not scans.ts's POST /scans/repository handler itself --
      // that route 404s when the passport doesn't already exist
      // (scans.ts:232-233). Here there is no pre-existing passport: the
      // repository-scan worker creates the passports row itself from the
      // acquired repo metadata (osv-worker.ts:426), keyed on this same
      // passportId, the first time it processes the job below.
  ...
  router.get('/free-review/scan/:passportId/status/:token', async (req, res, next) => {
    try {
      ...
      // Reads evidence_items / scan_findings directly, the same shape
      // GET /user/passports uses (auth.ts:533) -- this must NOT reuse the
      // public-connect.ts trust-response helper (public-connect.ts:23):
      // that helper reads trust_findings / evidence_ledger /
      // trust_observations, which are populated only by the
      // continuous-monitoring trust loop, never by this repository-scan
      // pipeline, and would silently report UNKNOWN for every Free Review
      // result.
      ...
      return res.json({
        ...
        findings,
        evidence,
        policy: { rule: 'SPR reports observed evidence only. A scan still in progress reports scanStatus "scanning", never a placeholder result.' },
      });
    } catch (error) { return next(error); }
  });
