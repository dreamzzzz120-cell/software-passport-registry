...
export async function publicTrustResponse(passport: any) {
  const findings = (await db.execute(sql`SELECT id,control_id,title,severity,status,updated_at,resolved_at FROM trust_findings WHERE tenant_id=${passport.tenant_id} AND passport_id=${passport.id} ORDER BY CASE severity WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 WHEN 'low' THEN 4 ELSE 5 END, updated_at DESC LIMIT 50`) as any).rows || [];
  const evidence = (await db.execute(sql`SELECT id,provider,control_id,subject,observed_at,verification_method,status,severity,evidence_hash,limitation FROM evidence_ledger WHERE tenant_id=${passport.tenant_id} AND passport_id=${passport.id} ORDER BY observed_at DESC LIMIT 100`) as any).rows || [];
  const observations = (await db.execute(sql`SELECT id,observation_version,generated_at,canonical_payload_hash,completeness_basis_points,open_finding_count,unknown_dimension_count FROM trust_observations WHERE tenant_id=${passport.tenant_id} AND passport_id=${passport.id} ORDER BY observation_version DESC LIMIT 1`) as any).rows || [];
