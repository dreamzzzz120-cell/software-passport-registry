...
export function createSoftwareRegistryRouter() {
  ...
  router.get('/sitemap.xml', async (_req: Request, res: Response, next) => {
    try {
      ...
      const urls = [`${PUBLIC_ORIGIN}/software`, `${PUBLIC_ORIGIN}/whitepaper`, `${PUBLIC_ORIGIN}/roi`, ...entries.map((e) => `${PUBLIC_ORIGIN}/software/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repository)}`)];
