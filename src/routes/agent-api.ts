...
const mintInput = z.object({
  name: z.string().trim().min(1).max(255), version: z.string().trim().min(1).max(255), publisher: z.string().trim().min(1).max(255),
  category: z.string().trim().min(1).max(120).default('software'),
  checksum: z.string().trim().regex(/^[a-fA-F0-9]{64}$/, 'checksum must be a SHA-256 hex digest'),
  releaseDate: z.string().trim().min(1).max(64).optional(), licenseType: z.string().trim().min(1).max(120).default('UNKNOWN'),
  clientId: z.string().trim().min(1).max(255).nullable().optional(),
}).strict();
const apiKeyCreateInput = z.object({
  name: z.string().trim().min(1).max(120),
  scopes: z.array(z.enum(['read', 'write', 'webhooks'])).min(1).max(3).refine((v) => new Set(v).size === v.length, 'scopes must be unique'),
...
function hasScope(req: AuthenticatedRequest, scope: 'read' | 'write' | 'webhooks') {
...
function requireScope(scope: 'read' | 'write' | 'webhooks') {
...
function validDate(value: string | undefined) { return value == null || Number.isFinite(Date.parse(value)); }
function futureOrNull(value: string | undefined) { return value == null || (Number.isFinite(Date.parse(value)) && Date.parse(value) > Date.now()); }
function isUniqueViolation(error: any) { return error?.code === '23505'; }
...
async function requireApiKey(req: AuthenticatedRequest, res: any, next: any) {
  ...
  try {
    ...
    const scopes = z.array(z.enum(['read', 'write', 'webhooks'])).safeParse(JSON.parse(String(row.scopes ?? '[]')));
    if (!scopes.success) return res.status(401).json({ error: { code: 'INVALID_API_KEY', message: 'API key configuration is invalid.' } });
    ...
    (req as any).sprApiKeyId = String(row.id); (req as any).sprApiKeyScopes = scopes.data;
...
function externalAuth(req: AuthenticatedRequest, res: any, next: any) {
  if (typeof req.headers['x-api-key'] === 'string') return requireApiKey(req, res, next);
...
export function createAgentApiRouter() {
  ...
  router.post('/api-keys', requireAuth, requireRole(['Owner', 'Admin']), async (req: AuthenticatedRequest, res, next) => {
    ...
    if (!parsed.success || !futureOrNull(parsed.success ? parsed.data.expiresAt : undefined)) return res.status(400).json({ error: 'INVALID_API_KEY_REQUEST', details: parsed.success ? { expiresAt: 'expiresAt must be a future timestamp.' } : parsed.error.flatten() });
    try {
  ...
  router.post('/passports', requireScope('write'), async (req: AuthenticatedRequest, res, next) => {
    ...
    if (!parsed.success || !validDate(parsed.success ? parsed.data.releaseDate : undefined)) return res.status(400).json({ error: 'INVALID_PASSPORT', details: parsed.success ? { releaseDate: 'releaseDate must be a valid date when supplied.' } : parsed.error.flatten() });
    try {
      const p = parsed.data;
      if (p.clientId) {
  ...
  router.post('/passports/verify', requireScope('read'), async (req: AuthenticatedRequest, res, next) => {
  ...
  router.get('/passports/:passportId/freshness', requireScope('read'), async (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      const generated = new Date(latest.generatedAt).getTime(); const ageDays = Number.isFinite(generated) ? (Date.now() - generated) / 86400000 : Number.POSITIVE_INFINITY;
  ...
  router.post('/verify-software', requireScope('read'), async (req: AuthenticatedRequest, res, next) => {
    ...
    try { const scoped = req.db!; const q = parsed.data.query.toLowerCase(); const passport = (await scoped.execute(sql`SELECT id,name,overall_score,security_score,compliance_score,evidence,vulnerabilities,timeline FROM passports WHERE tenant_id=${req.user!.tenantId} AND (LOWER(name)=${q} OR LOWER(id)=${q}) LIMIT 1`) as any).rows?.[0]; if (!passport) return res.status(404).json({ status: 'UNKNOWN', reason: 'SOFTWARE_NOT_REGISTERED', query: parsed.data.query }); return buildVerificationResponse(scoped, req.user!.tenantId, passport, res, next); } catch (error) { return next(error); }
  });
  router.post('/verify-passport', requireScope('read'), async (req: AuthenticatedRequest, res, next) => {
    const parsed = passportInput.safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: 'INVALID_PASSPORT_ID', details: parsed.error.flatten() });
    try { const scoped = req.db!; const passport = (await scoped.execute(sql`SELECT id,name,version,overall_score,security_score,compliance_score,evidence,vulnerabilities,timeline FROM passports WHERE tenant_id=${req.user!.tenantId} AND id=${parsed.data.passportId} LIMIT 1`) as any).rows?.[0]; if (!passport) return res.status(404).json({ status: 'UNKNOWN', reason: 'PASSPORT_NOT_FOUND', passportId: parsed.data.passportId }); return buildVerificationResponse(scoped, req.user!.tenantId, passport, res, next); } catch (error) { return next(error); }
  });
  router.get('/passport/:passportId', requireScope('read'), async (req: AuthenticatedRequest, res, next) => {
    try { const scoped = req.db!; const passport = (await scoped.execute(sql`SELECT id,name,version,overall_score,security_score,compliance_score,evidence,vulnerabilities,timeline FROM passports WHERE tenant_id=${req.user!.tenantId} AND id=${req.params.passportId} LIMIT 1`) as any).rows?.[0]; if (!passport) return res.status(404).json({ status: 'UNKNOWN', reason: 'PASSPORT_NOT_FOUND', passportId: req.params.passportId }); return buildVerificationResponse(scoped, req.user!.tenantId, passport, res, next); } catch (error) { return next(error); }
  });
  router.post('/vendor-risk', requireScope('read'), async (req: AuthenticatedRequest, res, next) => {
    ...
    try {
      const scoped = req.db!; const tenantId = req.user!.tenantId;
      const passport = (await scoped.execute(sql`SELECT id,name FROM passports WHERE tenant_id=${tenantId} AND id=${parsed.data.passportId} LIMIT 1`) as any).rows?.[0];
      if (!passport) return res.status(404).json({ status: 'UNKNOWN', reason: 'PASSPORT_NOT_FOUND', passportId: parsed.data.passportId });
      const findings = (await scoped.execute(sql`SELECT id,severity,status,title,updated_at FROM trust_findings WHERE tenant_id=${tenantId} AND passport_id=${passport.id} ORDER BY CASE severity WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 WHEN 'low' THEN 4 ELSE 5 END, updated_at DESC LIMIT 200`) as any).rows || [];
      const evidence = (await scoped.execute(sql`SELECT id,provider,observed_at,verification_method,status,limitation FROM evidence_ledger WHERE tenant_id=${tenantId} AND passport_id=${passport.id} ORDER BY observed_at DESC LIMIT 500`) as any).rows || [];
      const latest = (await scoped.execute(sql`SELECT generated_at,completeness_basis_points FROM trust_observations WHERE tenant_id=${tenantId} AND passport_id=${passport.id} ORDER BY observation_version DESC LIMIT 1`) as any).rows?.[0];
...
async function buildVerificationResponse(scopedDb: ScopedDb, tenantId: string, passport: any, res: any, next: any) {
  try {
    ...
    const latest = observations[0]; const openFindings = findings.filter((f: any) => !['resolved','closed','verified'].includes(String(f.status).toLowerCase())); const criticalOrHigh = openFindings.filter((f: any) => ['critical','high'].includes(String(f.severity).toLowerCase()));
