        ...
        <div className="max-h-[560px] divide-y divide-[var(--spr-border)] overflow-y-auto">
          {loading ? <div className="p-6 text-sm text-[var(--spr-text-muted)]">Loading…</div> : filtered.length === 0 ? (
            ...
            <button key={r.id} onClick={() => setSelectedId(r.id)} className={`flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm transition ${selectedId === r.id ? 'bg-[var(--spr-accent-soft)]/45' : 'hover:bg-[var(--spr-surface-sunken)]'}`}>
              ...
              <span className={`shrink-0 rounded-sm border px-2 py-0.5 text-[12px] font-bold uppercase ${STATUS_STYLE[r.acceptanceStatus]}`}>{r.acceptanceStatus}</span>
            </button>
          ))}
        </div>
      </section>
        ...
        {!selected ? <p className="py-16 text-center text-sm text-[var(--spr-text-muted)]">Select a risk to view details.</p> : (
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={`shrink-0 rounded-sm border px-2 py-0.5 text-[12px] font-bold uppercase ${STATUS_STYLE[selected.acceptanceStatus]}`}>{selected.acceptanceStatus}</span>
            </div>
            <p className="text-xs text-[var(--spr-text-muted)]">{selected.description || 'No description provided.'}</p>
            <div className="grid gap-3 sm:grid-cols-2 text-xs">
              <div className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-deep)] p-3"><div className="text-[12px] font-bold uppercase text-[var(--spr-text-faint)]">Inherent risk</div><p className="mt-1"><span className={LEVEL_STYLE[selected.likelihood]}>{selected.likelihood}</span> likelihood × <span className={LEVEL_STYLE[selected.impact]}>{selected.impact}</span> impact</p></div>
              <div className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-deep)] p-3"><div className="text-[12px] font-bold uppercase text-[var(--spr-text-faint)]">Residual risk</div><p className="mt-1">{selected.residualLikelihood && selected.residualImpact ? <>{selected.residualLikelihood} × {selected.residualImpact}</> : <span className="italic text-[var(--spr-text-faint)]">Not yet assessed</span>}</p></div>
            </div>
            {selected.mitigation && <div><div className="text-[12px] font-bold uppercase tracking-wider text-[var(--spr-text-faint)]">Mitigation</div><p className="mt-1 text-xs text-[var(--spr-text-muted)]">{selected.mitigation}</p></div>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-6 shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="mt-5 space-y-3">
              ...
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Title *<input required value={form.title} onChange={(e) => setForm((c) => ({ ...c, title: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <div className="grid grid-cols-2 gap-3">
                <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Likelihood<select value={form.likelihood} onChange={(e) => setForm((c) => ({ ...c, likelihood: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]">{['LOW', 'MEDIUM', 'HIGH'].map((l) => <option key={l} value={l}>{l}</option>)}</select></label>
                <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Impact<select value={form.impact} onChange={(e) => setForm((c) => ({ ...c, impact: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]">{['LOW', 'MEDIUM', 'HIGH'].map((l) => <option key={l} value={l}>{l}</option>)}</select></label>
              </div>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Description<textarea value={form.description} onChange={(e) => setForm((c) => ({ ...c, description: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Owner<input value={form.ownerName} onChange={(e) => setForm((c) => ({ ...c, ownerName: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-6 shadow-2xl">
            ...
            <form onSubmit={handleAccept} className="mt-4 space-y-3">
              ...
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Decision<select value={acceptForm.status} onChange={(e) => setAcceptForm((c) => ({ ...c, status: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]">{['ACCEPTED', 'MITIGATED', 'TRANSFERRED'].map((s) => <option key={s} value={s}>{s}</option>)}</select></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Authorized by *<input required value={acceptForm.acceptedBy} onChange={(e) => setAcceptForm((c) => ({ ...c, acceptedBy: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Rationale *<textarea required value={acceptForm.acceptanceRationale} onChange={(e) => setAcceptForm((c) => ({ ...c, acceptanceRationale: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Scope *<input required value={acceptForm.acceptanceScope} onChange={(e) => setAcceptForm((c) => ({ ...c, acceptanceScope: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Review date *<input required type="date" value={acceptForm.reviewDate} onChange={(e) => setAcceptForm((c) => ({ ...c, reviewDate: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <div className="flex justify-end gap-2 pt-2">
