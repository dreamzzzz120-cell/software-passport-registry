...
export default function GovernanceAuditTab() {
  ...
  return (
    <div className="spr-panel">
      <div className="flex flex-col gap-2 border-b border-[var(--spr-border)] p-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search action, actor, or details… (searches records already loaded)" className="w-full rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] py-1.5 pl-8 pr-2 text-xs text-[var(--spr-text)]" />
        </div>
        <label className="flex items-center gap-1.5 text-xs text-[var(--spr-text-muted)]"><input type="checkbox" checked={governanceOnly} onChange={(e) => setGovernanceOnly(e.target.checked)} /> Governance actions only</label>
      </div>
      ...
      {loading ? <div className="p-6 text-sm text-[var(--spr-text-muted)]">Loading…</div> : filtered.length === 0 ? (
        <div className="p-8 text-center text-sm text-[var(--spr-text-muted)]"><ClipboardList className="mx-auto h-7 w-7 text-[var(--spr-text-faint)]" /><p className="mt-2 font-semibold text-[var(--spr-text)]">No audit entries match this search.</p></div>
      ) : (
            ...
            return (
              <div key={r.id} className="px-4 py-3 text-xs">
                <div className="flex items-center justify-between gap-3">
                  <span className="font-semibold text-[var(--spr-text)]">{r.action}</span>
      ...
      {hasMore && !loading && (
        <div className="border-t border-[var(--spr-border)] p-3 text-center">
