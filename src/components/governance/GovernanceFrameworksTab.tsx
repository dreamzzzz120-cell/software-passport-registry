      ...
      <section className="lg:col-span-2 spr-panel divide-y divide-[var(--spr-border)] overflow-hidden">
        ...
        {loading ? <div className="p-6 text-sm text-[var(--spr-text-muted)]">Loading…</div> : frameworks.map((f) => (
          <button key={f.id} onClick={() => void loadRequirements(f.id)} className={`flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm transition ${selectedId === f.id ? 'bg-[var(--spr-accent-soft)]/45' : 'hover:bg-[var(--spr-surface-sunken)]'}`}>
            ...
            <span className="shrink-0 rounded-sm border border-[var(--spr-border)] px-2 py-0.5 text-[11px] font-bold uppercase text-[var(--spr-text-muted)]">{f.status.replace(/_/g, ' ')}</span>
          </button>
        ))}
      </section>
                ...
                {requirements.map((req) => (
                  <div key={req.id} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-deep)] p-3.5">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`shrink-0 rounded-sm border px-1.5 py-0.5 text-[11px] font-bold uppercase ${req.status === 'VERIFIED_SOURCE' ? 'border-[var(--spr-green)]/40 text-[var(--spr-green)]' : 'border-[var(--spr-border)] text-[var(--spr-text-muted)]'}`}>{req.status.replace(/_/g, ' ')}</span>
                    </div>
                    {req.requirementText && <p className="mt-1.5 text-xs text-[var(--spr-text-muted)]">{req.requirementText}</p>}
                    {req.authoritativeSource && <p className="mt-1 text-[12px] text-[var(--spr-text-faint)]">Source: {req.authoritativeSource}</p>}
                    {canMap && (
                      <div className="mt-2 flex items-center gap-2">
                        <span className="text-[12px] font-bold uppercase text-[var(--spr-text-faint)]">Our status:</span>
                        <select disabled={mappingSaving === req.id} value={req.tenantStatus} onChange={(e) => void handleMappingChange(req.id, e.target.value)} className={`rounded-sm border bg-[var(--spr-surface-sunken)] px-1.5 py-0.5 text-[12px] font-bold uppercase ${MAPPING_STYLE[req.tenantStatus]}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-6 shadow-2xl">
            ...
            <form onSubmit={handleAddRequirement} className="mt-5 space-y-3">
              ...
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Requirement key * (unique within this framework)<input required value={reqForm.requirementKey} onChange={(e) => setReqForm((c) => ({ ...c, requirementKey: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Requirement text<textarea value={reqForm.requirementText} onChange={(e) => setReqForm((c) => ({ ...c, requirementText: e.target.value }))} rows={3} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Authoritative source (a real citation -- required to mark this as a verified source)<input value={reqForm.authoritativeSource} onChange={(e) => setReqForm((c) => ({ ...c, authoritativeSource: e.target.value }))} placeholder="e.g. NIST SP 800-53 Rev 5, Section AC-2" className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Jurisdiction<input value={reqForm.jurisdiction} onChange={(e) => setReqForm((c) => ({ ...c, jurisdiction: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
