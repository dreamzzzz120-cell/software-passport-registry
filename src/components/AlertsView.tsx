...
import { AlertTriangle, Bell, CheckCircle2, Clock3, Filter, Search, ShieldAlert, ShieldCheck, X } from 'lucide-react';
...
export default function AlertsView({ alerts, onUpdateAlertStatus }: AlertsViewProps) {
  ...
  const criticalCount = alerts.filter((alert) => alert.status === 'Active' && alert.severity === 'Critical').length;
  ...
  return (
    <section className="space-y-6" id="msp-alerts-hub">
      <header className="rounded-[28px] border border-white/10 bg-white/[.035] p-6 backdrop-blur-2xl md:p-8">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Review server-backed trust findings and record an explicit workflow state. Alert copy and timestamps remain tied to the source record.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
            ...
            <SelectFilter icon={<Filter />} label="Status" value={statusFilter} onChange={setStatusFilter} options={['all', 'Active', 'Snoozed', 'Resolved']} />
          </div>
        </div>
        <div className="mt-7 grid gap-3 sm:grid-cols-3">
          <Metric label="Active attention" value={activeCount} tone="rose" icon={<ShieldAlert />} />
          <Metric label="Critical active" value={criticalCount} tone="amber" icon={<AlertTriangle />} />
      ...
      {selectedAlert && <AlertDrawer alert={selectedAlert} onClose={() => setSelectedAlertId(null)} onUpdateAlertStatus={onUpdateAlertStatus} />}
    </section>
  );
}
...
function AlertDrawer({ alert, onClose, onUpdateAlertStatus }: { alert: Alert; onClose: () => void; onUpdateAlertStatus: (id: string, status: AlertStatus) => void }) {
  const update = (status: AlertStatus) => { onUpdateAlertStatus(alert.id, status); onClose(); };
  return <div className="fixed inset-0 z-[70] flex justify-end bg-black/60 backdrop-blur-sm" onMouseDown={onClose}>
    <aside role="dialog" aria-modal="true" aria-label={`Alert details for ${alert.title}`} onMouseDown={(event) => event.stopPropagation()} className="h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-[#0a0f18] p-6 shadow-2xl md:p-8">
      ...
      <div className="mt-6 space-y-5"><div className="rounded-2xl border border-white/[.08] bg-white/[.025] p-5"><div className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-600">Description</div><p className="mt-3 text-sm leading-7 text-slate-300">{alert.description || 'No description observed.'}</p></div><div className="grid gap-3 sm:grid-cols-2"><Detail label="Observed timestamp" value={alert.timestamp || 'Not observed'} /><Detail label="Current workflow state" value={alert.status} /></div></div>
      <div className="mt-8 rounded-2xl border border-white/[.08] bg-white/[.025] p-5"><div className="text-xs font-semibold text-white">Record a workflow decision</div><p className="mt-1 text-xs leading-5 text-slate-500">This updates the alert state through the authenticated Trust Loop route.</p><div className="mt-5 flex flex-wrap gap-2">{alert.status === 'Active' && <><button onClick={() => update('Snoozed')} className="rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/[.05]">Snooze</button><button onClick={() => update('Resolved')} className="rounded-xl bg-emerald-400 px-3 py-2 text-xs font-bold text-slate-950">Mark resolved</button></>}{alert.status === 'Snoozed' && <button onClick={() => update('Active')} className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-xs font-semibold text-cyan-200">Restore active</button>}{alert.status === 'Resolved' && <div className="flex items-center gap-2 text-xs text-emerald-200"><ShieldCheck className="h-4 w-4" />Resolved state recorded; new evidence is still required to verify the finding.</div>}</div></div>
    </aside>
  </div>;
}
