...
import { AlertCircle, ArrowUpDown, Check, ChevronRight, ExternalLink, FileText, HelpCircle, Lock, Plus, Search, X } from 'lucide-react';
...
export default function VendorsView({ vendors: initialVendors, searchQuery: globalSearchQuery, role = 'Viewer' }: VendorsViewProps) {
  ...
  const handleAddAuditAttestation = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedVendor || !canLodgeAudit || auditBusy) return;
    setAuditBusy(true);
    setError(null);
    try {
