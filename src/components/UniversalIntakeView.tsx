...
const MAX_FILES = 100;
// Must match MAX_FILE_SIZE in src/routes/universal-intake.ts -- that value is
// bounded by the Supabase project's own storage cap, not a number this app
// chooses freely, so this client-side check would otherwise accept a file the
// server then rejects.
...
export default function UniversalIntakeView({ onContinue }: { onContinue?: () => void }) {
  ...
  const addFiles = (files: FileList | File[]) => {
    ...
    for (const file of Array.from(files).slice(0, MAX_FILES)) {
      if (file.size > MAX_SIZE) { rejected.push(`${file.name}: over 50 MB`); continue; }
  ...
  return <div className="min-h-screen bg-[var(--spr-surface)] px-5 py-10 text-[var(--spr-text)]"><div className="mx-auto max-w-6xl">
    <div className="mb-10 max-w-3xl"><div className="inline-flex items-center gap-2 rounded-full border border-[var(--spr-highlight)]/30 bg-[var(--spr-accent-soft)]/20 px-3 py-1.5 text-[12px] font-bold uppercase tracking-[.18em] text-[var(--spr-highlight)]"><Sparkles className="h-3.5 w-3.5" /> Universal MSP Intake</div><h1 className="mt-5 text-4xl font-semibold tracking-tight md:text-6xl">VERIFY EVERYTHING.</h1><p className="mt-5 text-base leading-7 text-[var(--spr-text-muted)] md:text-lg">Give SPR what you already have. Connect a repository, upload a complete project, or bring policies, contracts, security, privacy and compliance evidence.</p><p className="mt-3 text-sm font-semibold">You do not need to make an SBOM first.</p></div>
