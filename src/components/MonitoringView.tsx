...
const SEVERITY_STYLE: Record<string, string> = {
  critical: 'text-[#f14c4c] border-[#f14c4c]/40', high: 'text-[#f14c4c] border-[#f14c4c]/40',
  medium: 'text-[#cca700] border-[#cca700]/40', low: 'text-[#9d9d9d] border-[#3c3c3c]',
  informational: 'text-[#9d9d9d] border-[#3c3c3c]',
};
...
export default function MonitoringView({ role = 'Viewer', passports = [], clients = [] }: { role?: string; passports?: SoftwarePassport[]; clients?: Client[] }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-7 pb-10" id="monitoring-workspace">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.2em] text-[#3794ff]"><Activity className="h-4 w-4" /> Continuous verification</div><h1 className="mt-3 text-3xl font-bold tracking-tight text-[#d4d4d4]">What changed since the last check?</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#9d9d9d]">Monitoring queues configured server-side collectors. A queued run is not a completed or verified result.</p></div>
    ...
    {monitoringDisabled && <div className="flex gap-3 rounded-md border border-[#3c3c3c] bg-[#252526] p-4 text-sm text-[#cca700]"><AlertCircle className="h-5 w-5 shrink-0" /><div><p className="font-semibold">Monitoring is not enabled for this workspace</p><p className="mt-1 text-[#9d9d9d]">Ask an Owner to enable it for this tenant.</p></div></div>}
    {error && <div role="alert" className="flex gap-3 rounded-md border border-[#3c3c3c] bg-[#252526] p-4 text-sm text-[#f14c4c]"><AlertCircle className="h-5 w-5 shrink-0" /><div><p className="font-semibold">Verification unavailable</p><p className="mt-1 text-[#f14c4c]">{error}</p></div></div>}

    {!monitoringDisabled && !loading && (
      <section className="spr-panel p-5">
        <div className="flex items-center justify-between"><h2 className="text-sm font-bold uppercase tracking-wider text-[#d4d4d4]">Alerts{openAlerts.length > 0 ? ` (${openAlerts.length} open)` : ''}</h2></div>
        {alerts.length === 0
          ? <p className="mt-3 text-sm text-[#9d9d9d]">No changes have triggered an alert yet. Alerts appear here automatically when a monitored source regresses.</p>
          : <ul className="mt-3 divide-y divide-[#3c3c3c]">{alerts.slice(0, 25).map((alert) => (
              <li key={alert.id} className="flex items-start justify-between gap-4 py-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-[#6f6f6f]">{alert.status}</span>
                  </div>
                  <p className="mt-1.5 text-sm text-[#d4d4d4]">{alert.message}</p>
                  <p className="mt-0.5 text-xs text-[#6f6f6f]">{readable(alert.created_at)}</p>
                </div>
                {canManageAlerts && (alert.status === 'OPEN' || alert.status === 'ACKNOWLEDGED') && (
                  <div className="flex shrink-0 gap-1.5">
                    {alert.status === 'OPEN' && <button onClick={() => void updateAlert(alert.id, 'ACKNOWLEDGED')} disabled={alertUpdating === alert.id} className="rounded-md border border-[#3c3c3c] px-2.5 py-1 text-xs font-semibold text-[#9d9d9d] hover:bg-[#383838] disabled:opacity-50">Acknowledge</button>}
                    <button onClick={() => void updateAlert(alert.id, 'RESOLVED')} disabled={alertUpdating === alert.id} className="rounded-md border border-[#3c3c3c] px-2.5 py-1 text-xs font-semibold text-[#89d185] hover:bg-[#383838] disabled:opacity-50">Resolve</button>
                  </div>
                )}
              </li>
            ))}</ul>}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-labelledby="enroll-title">
        <div className="w-full max-w-md rounded-md border border-[#3c3c3c] bg-[#252526] p-6 shadow-2xl">
          <div className="flex items-start justify-between gap-4">
            <div><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.18em] text-[#3794ff]"><Activity className="h-4 w-4" /> New monitor</div><h2 id="enroll-title" className="mt-1 text-lg font-bold text-[#d4d4d4]">Enable monitoring</h2></div>
            <button onClick={() => setShowEnroll(false)} aria-label="Close" className="rounded-md p-1.5 text-[#9d9d9d] hover:bg-[#383838] hover:text-[#d4d4d4]"><XCircle className="h-4 w-4" /></button>
          </div>
          <form onSubmit={handleEnroll} className="mt-5 space-y-3.5">
            {enrollError && <div role="alert" className="rounded-md border border-[#f14c4c]/40 bg-[#f14c4c]/10 px-3 py-2.5 text-xs text-[#f14c4c] flex items-center gap-2"><AlertCircle className="w-4 h-4 shrink-0" /> {enrollError}</div>}
            {enrollSuccess && <div className="rounded-md border border-[#89d185]/40 bg-[#89d185]/10 px-3 py-2.5 text-xs text-[#89d185] flex items-center gap-2"><CheckCircle2 className="w-4 h-4 shrink-0" /> {enrollSuccess}</div>}

            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-[#9d9d9d]">Client *</label>
              <select required value={enrollClientId} onChange={(e) => setEnrollClientId(e.target.value)} className="rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-3 py-2 text-xs text-[#d4d4d4]">
                <option value="">{clients.length ? 'Select client…' : 'No clients yet — add one first'}</option>
                {clients.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-[#9d9d9d]">Software / Passport *</label>
              <select required value={enrollPassportId} onChange={(e) => setEnrollPassportId(e.target.value)} className="rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-3 py-2 text-xs text-[#d4d4d4]">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-[#9d9d9d]">Source type *</label>
              <select value={enrollCollectorId} onChange={(e) => { setEnrollCollectorId(e.target.value); setEnrollSubject(''); }} className="rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-3 py-2 text-xs text-[#d4d4d4]">
                {ENROLLABLE_COLLECTORS.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-[#9d9d9d]">{activeCollector.label} identifier *</label>
              <input required value={enrollSubject} onChange={(e) => setEnrollSubject(e.target.value)} placeholder={activeCollector.placeholder} className="rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-3 py-2 text-xs text-[#d4d4d4]" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-[#9d9d9d]">Check frequency</label>
              <select value={enrollSchedule} onChange={(e) => setEnrollSchedule(Number(e.target.value))} className="rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-3 py-2 text-xs text-[#d4d4d4]">
                {availableSchedules.map((s) => <option key={s.seconds} value={s.seconds}>{s.label}</option>)}
              </select>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setShowEnroll(false)} className="rounded-md border border-[#3c3c3c] px-3.5 py-2 text-xs font-semibold text-[#9d9d9d] hover:bg-[#383838]">Cancel</button>
              <button type="submit" disabled={enrolling || !enrollClientId || !enrollPassportId || !enrollSubject.trim()} className="inline-flex items-center gap-1.5 rounded-md bg-[#0e639c] px-3.5 py-2 text-xs font-bold text-white hover:bg-[#1177bb] disabled:opacity-40">
...
function Row({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex gap-3"><span className="h-4 w-4 text-[#6f6f6f]">{icon}</span><div><dt className="text-xs text-[#6f6f6f]">{label}</dt><dd className="mt-0.5 text-[#d4d4d4]">{value}</dd></div></div>; }
