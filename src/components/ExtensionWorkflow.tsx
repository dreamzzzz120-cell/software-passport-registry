...
function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl border border-white/[.07] bg-white/[.035] p-4 backdrop-blur-xl"><div className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-600">{label}</div><div className="mt-2 text-2xl font-semibold tracking-tight text-white">{value}</div></div>;
}
...
export default function ExtensionWorkflow({ id, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const [passports, scans, findings, clients, integrations] = await Promise.all([
      ...
      const values = await Promise.all([passports, scans, findings, clients, integrations].map(readCount));
  ...
  return <section className="space-y-6">
    <div className="relative overflow-hidden rounded-3xl border border-white/[.08] bg-white/[.035] p-6 shadow-2xl backdrop-blur-2xl md:p-8">
      ...
      <div className="relative"><div className="mb-4 flex flex-wrap items-center gap-2"><span className="rounded-full border border-violet-300/20 bg-violet-300/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.18em] text-violet-200">Extension workflow</span><span className="rounded-full border border-emerald-300/15 bg-emerald-300/[.05] px-2.5 py-1 text-[10px] text-emerald-200">Evidence connected</span></div><h1 className="max-w-3xl text-3xl font-semibold tracking-tight md:text-4xl">{extension.name}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">{extension.description}</p><div className="mt-6 flex flex-wrap gap-3"><button onClick={() => onNavigate(firstRoute)} className="rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-semibold text-slate-950 shadow-[0_0_28px_rgba(34,211,238,.16)] hover:bg-cyan-200">Open live workflow</button><button onClick={() => setActiveStep((step) => Math.min(step + 1, extension.steps.length - 1))} className="rounded-xl border border-white/10 bg-white/[.035] px-4 py-2.5 text-sm text-slate-200 hover:bg-white/[.06]">Advance workflow</button></div></div>
    </div>

    <div className="grid gap-3 md:grid-cols-3">{metrics.map(([label, value]) => <Metric key={label} label={label} value={loading ? 0 : value} />)}</div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="rounded-3xl border border-white/[.07] bg-white/[.025] p-5 backdrop-blur-xl"><div className="text-sm font-semibold">Live evidence surface</div><p className="mt-1 text-xs leading-5 text-slate-600">Counts below are read from the authenticated API surface. No demo records are injected by this UI.</p><div className="mt-5 space-y-3 text-xs">{[['Passports', snapshot.passports, '/passports'], ['Scans', snapshot.scans, '/scans'], ['Findings', snapshot.findings, '/alerts'], ['Clients', snapshot.clients, '/clients']].map(([label, value, path]) => <button key={String(label)} onClick={() => onNavigate(String(path))} className="flex w-full items-center justify-between rounded-xl border border-white/[.06] bg-black/10 px-3 py-3 text-left hover:border-white/15"><span className="text-slate-400">{label}</span><span className="font-semibold text-white">{String(value)} <span className="ml-2 text-slate-600">→</span></span></button>)}</div></div>
    </div>
  </section>;
}
