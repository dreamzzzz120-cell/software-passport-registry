...
export default function ExtensionWorkflow({ id, onNavigate }: Props) {
  ...
  if (!extension) return <div className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-8 text-[var(--spr-red)]">Capability not found.</div>;
  if (id === 'new-review') return <NewReviewIntake />;
  if (id === 'msp-command-center') return <MSPStackCommandCenter onNavigate={onNavigate} />;
  if (unauthorized) return <div className="spr-panel p-8"><div className="text-[12px] font-bold uppercase tracking-[.2em] text-[var(--spr-amber)]">Session expired</div><h1 className="mt-2 text-2xl font-semibold text-[var(--spr-text)]">Re-authentication required</h1><p className="mt-2 text-sm text-[var(--spr-text-muted)]">The capability could not access its protected evidence sources.</p><button onClick={() => onNavigate('/login')} className="spr-btn spr-btn-primary mt-5">Return to sign in</button></div>;

  const firstRoute = extension.sourceRoutes[0] || '/dashboard';
  return <section className="space-y-6">
    <div className="spr-panel p-6 md:p-8">
      <div><div className="mb-4 flex flex-wrap items-center gap-2"><span className="rounded-sm border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] px-2.5 py-1 text-[12px] font-bold uppercase tracking-[.18em] text-[#4ec9b0]">SPR capability</span><span className="rounded-sm border border-[var(--spr-border)] px-2.5 py-1 text-[12px] text-[var(--spr-green)]">Uses canonical evidence</span></div><h1 className="max-w-3xl text-3xl font-semibold tracking-tight text-[var(--spr-text)] md:text-4xl">{extension.name}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--spr-text-muted)]">{extension.description}</p><div className="mt-6 flex flex-wrap gap-3"><button onClick={() => onNavigate(firstRoute)} className="spr-btn spr-btn-primary">Open canonical workflow</button><button onClick={() => setActiveStep((step) => Math.min(step + 1, extension.steps.length - 1))} className="spr-btn spr-btn-secondary">Advance</button></div></div>
    </div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      <div className="spr-panel p-5"><div className="mb-4"><div className="text-sm font-semibold text-[var(--spr-text)]">Workflow</div><div className="mt-1 text-xs text-[var(--spr-text-faint)]">Capabilities link into canonical SPR surfaces instead of creating duplicate records or workflows.</div></div><div className="space-y-2">{extension.steps.map((step, index) => <Step key={step} index={index} label={step} active={index === activeStep} />)}</div></div>
      <div className="spr-panel p-5"><div className="text-sm font-semibold text-[var(--spr-text)]">Evidence summary</div><p className="mt-1 text-xs leading-5 text-[var(--spr-text-faint)]">Counts come only from protected API responses. Unavailable sources are shown as <span className="text-[var(--spr-text-muted)]">Not verified</span>; SPR never infers a value.</p>{loadFailed && <div className="mt-3 rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-3 text-xs text-[var(--spr-red)]">Evidence sources could not be loaded. No values were inferred.</div>}<div className="mt-5 space-y-3 text-xs">{[['Passports', snapshot.passports, '/passports'], ['Scans', snapshot.scans, '/scans'], ['Findings', snapshot.findings, '/alerts'], ['Clients', snapshot.clients, '/clients']].map(([label, value, path]) => <button key={String(label)} onClick={() => onNavigate(String(path))} className="flex w-full items-center justify-between rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-3 text-left hover:bg-[var(--spr-surface-hover)]"><span className="text-[var(--spr-text-muted)]">{label}</span><span className="font-semibold text-[var(--spr-text)]">{loading ? '—' : value === null ? 'Not verified' : String(value)} <span className="ml-2 text-[var(--spr-text-faint)]">→</span></span></button>)}</div></div>
    </div>
  </section>;
}
