...
function Metric({ label, value, loading }: MetricProps) {
  ...
  return <div className="rounded-md border border-[#3c3c3c] bg-[#252526] p-4"><div className="text-[10px] font-bold uppercase tracking-[.18em] text-[#6f6f6f]">{label}</div><div className="mt-2 text-2xl font-semibold tracking-tight text-[#d4d4d4]">{display}</div></div>;
}

type StepProps = { index: number; label: string; active: boolean; key?: Key };
function Step({ index, label, active }: StepProps) {
  return <div className={`flex items-center gap-3 rounded-md border border-[#3c3c3c] px-3 py-3 ${active ? 'bg-[#094771]' : 'bg-[#2d2d2d]'}`}><span className={`grid h-7 w-7 place-items-center rounded-full text-[10px] font-bold ${active ? 'bg-[#0e639c] text-white' : 'bg-[#383838] text-[#9d9d9d]'}`}>{index + 1}</span><span className={active ? 'text-sm text-[#d4d4d4]' : 'text-sm text-[#9d9d9d]'}>{label}</span>{active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[#3794ff]" />}</div>;
}
...
export default function ExtensionWorkflow({ id, onNavigate }: Props) {
  ...
  if (!extension) return <div className="rounded-md border border-[#3c3c3c] bg-[#252526] p-8 text-[#f14c4c]">Extension not found.</div>;
  if (unauthorized) return <div className="spr-panel p-8"><div className="text-[10px] font-bold uppercase tracking-[.2em] text-[#cca700]">Session expired</div><h1 className="mt-2 text-2xl font-semibold text-[#d4d4d4]">Re-authentication required</h1><p className="mt-2 text-sm text-[#9d9d9d]">The extension could not access its protected evidence sources.</p><button onClick={() => onNavigate('/login')} className="spr-btn spr-btn-primary mt-5">Return to sign in</button></div>;
  ...
  return <section className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="spr-panel p-5"><div className="text-sm font-semibold text-[#d4d4d4]">Observed evidence surface</div><p className="mt-1 text-xs leading-5 text-[#6f6f6f]">Counts are displayed only when the protected API returns a recognized collection. Unavailable sources are explicitly marked <span className="text-[#9d9d9d]">Not verified</span>.</p>{loadFailed && <div className="mt-3 rounded-md border border-[#3c3c3c] bg-[#252526] p-3 text-xs text-[#f14c4c]">Evidence sources could not be loaded. No values are inferred.</div>}<div className="mt-5 space-y-3 text-xs">{[['Passports', snapshot.passports, '/passports'], ['Scans', snapshot.scans, '/scans'], ['Findings', snapshot.findings, '/alerts'], ['Clients', snapshot.clients, '/clients']].map(([label, value, path]) => <button key={String(label)} onClick={() => onNavigate(String(path))} className="flex w-full items-center justify-between rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-3 py-3 text-left hover:bg-[#383838]"><span className="text-[#9d9d9d]">{label}</span><span className="font-semibold text-[#d4d4d4]">{loading ? '—' : value === null ? 'Not verified' : String(value)} <span className="ml-2 text-[#6f6f6f]">→</span></span></button>)}</div></div>
    </div>
  </section>;
}
