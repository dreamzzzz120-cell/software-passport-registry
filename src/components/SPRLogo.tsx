    ...
    <div className={`flex items-center gap-3 select-none ${className}`} id="spr-brand-logo-root">
      ...
      <div className={`relative flex items-center justify-center shrink-0 ${iconSizes[size]}`}>
        ...
        >
          <defs>
            ...
            <linearGradient id="sprEmeraldGlow" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="var(--spr-green)" />
          ...
          <circle cx="50" cy="14" r="3.5" fill="var(--spr-green)" />
          <circle cx="82" cy="30" r="3.5" fill="#fbbf24" />
          <circle cx="82" cy="70" r="3.5" fill="#60a5fa" />
          <circle cx="50" cy="86" r="3.5" fill="var(--spr-green)" />
        ...
        <span className="absolute -top-0.5 -right-0.5 flex h-2.5 w-2.5">
          ...
          <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500 border border-[var(--spr-border)]"></span>
        </span>
      </div>
      ...
      {showText && variant !== 'icon' && (
        <div className="flex flex-col leading-none">
          <div className="flex items-center gap-1.5">
            <span className={`font-display font-black tracking-tight text-[var(--spr-text)] ${textSizes[size]}`}>
              SPR
            </span>
            <span className="bg-[var(--spr-accent-soft)] text-[var(--spr-highlight)] border border-[var(--spr-highlight)]/40 font-mono text-[9px] font-bold px-1.5 py-0.5 rounded tracking-wider uppercase">
              TRUST-OS
            </span>
          </div>
          {subtext && (
            <span className="text-[8px] font-mono font-bold tracking-widest text-[var(--spr-text-muted)] uppercase mt-1">
              {subtext}
            </span>
          )}
        </div>
