...
export default function InvestorHomeView({ passports, onShowTelemetry, onNavigateTab, clients = [], alerts = [] }: InvestorHomeViewProps) {
  ...
  return <div className="space-y-8 py-4">
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-8 text-left">
      {!hasRegistryData && <div className="spr-panel-alt border-dashed p-5"><p className="text-sm font-extrabold text-[#d4d4d4]">No registry records are available yet.</p><p className="mt-1 text-[11px] leading-relaxed text-[#9d9d9d]">The dashboard is showing current dataset counts only. No absence of records is treated as proof of safety.</p></div>}

      <div className="flex flex-col justify-between gap-4 border-b border-[#3c3c3c] pb-5 md:flex-row md:items-center"><div><span className="mb-1 block text-[11px] font-semibold uppercase tracking-[.06em] text-[#6f6f6f]">CURRENT DATASET</span><h1 className="text-2xl font-black tracking-tight text-[#d4d4d4] sm:text-3xl">Investor & Evidence View</h1><p className="mt-1.5 max-w-2xl text-xs leading-relaxed text-[#9d9d9d]">Decision support derived from the records currently returned by SPR. It is not a certification or attestation.</p></div><button onClick={onShowTelemetry} className="spr-btn spr-btn-secondary flex shrink-0 items-center gap-1"><span>View Verification Telemetry</span><ArrowRight className="h-3.5 w-3.5" /></button></div>

      <div className="grid grid-cols-1 items-stretch gap-6 lg:grid-cols-12">
        <div className="relative flex min-h-[300px] flex-col justify-between overflow-hidden spr-panel p-6 text-[#d4d4d4] lg:col-span-5"><div><span className="block text-[10px] font-semibold uppercase tracking-[.06em] text-[#3794ff]">TRUST SCORE OBSERVATION</span><h3 className="text-sm font-black text-[#d4d4d4]">Client Trust Index</h3></div><div className="flex items-center justify-around gap-6 py-2"><div className="relative flex h-36 w-36 items-center justify-center"><svg className="h-full w-full -rotate-90"><circle cx="72" cy="72" r="60" className="text-[#3c3c3c]" strokeWidth="8" stroke="currentColor" fill="transparent"/><circle cx="72" cy="72" r="60" className={trustHealth === null ? 'text-[#6f6f6f]' : 'text-[#3794ff]'} strokeWidth="8" strokeDasharray={377} strokeDashoffset={trustHealth === null ? 377 : 377 - (377 * trustHealth) / 100} strokeLinecap="round" stroke="currentColor" fill="transparent"/></svg><div className="absolute flex flex-col items-center justify-center text-center"><span className="text-3xl font-black">{trustHealth === null ? '—' : trustHealth}</span><span className="text-[8px] font-bold uppercase tracking-wider text-[#9d9d9d]">Observed score</span></div></div><div className="space-y-2 font-mono text-[10px] text-[#9d9d9d]"><div className="flex justify-between gap-4 border-b border-[#3c3c3c] pb-1"><span>SCORE STATUS:</span><strong className="text-[#d4d4d4]">{trustHealth === null ? 'NOT VERIFIED' : 'OBSERVED'}</strong></div><p className="pt-1 text-[9px] leading-normal text-[#6f6f6f]">{trustHealth === null ? 'No client trust scores were returned by the backend.' : 'Average of returned client trust scores.'}</p></div></div><div className="border-t border-[#3c3c3c] pt-3 text-[9px] font-mono text-[#6f6f6f]">External assurance, certifications, and infrastructure claims require separately connected evidence.</div></div>
      ...
      <section className="spr-panel p-6"><div className="flex flex-col justify-between gap-3 border-b border-[#3c3c3c] pb-4 sm:flex-row sm:items-center"><div><h3 className="text-sm font-extrabold text-[#d4d4d4]">Software Passports</h3><p className="text-[11px] leading-tight text-[#9d9d9d]">Current passport records. Scores are shown only when returned by the backend.</p></div><div className="flex items-center gap-2 rounded-md border border-[#3c3c3c] bg-[#2d2d2d] px-2.5 py-1.5"><Search className="h-3.5 w-3.5 text-[#6f6f6f]"/><input aria-label="Filter passports" type="text" placeholder="Filter passports..." value={passportSearch} onChange={(e) => setPassportSearch(e.target.value)} className="w-full bg-transparent text-[11px] outline-none text-[#d4d4d4]"/></div></div><div className="mt-4 overflow-x-auto"><table className="spr-table w-full"><thead><tr><th>Software</th><th>Category</th><th className="text-center">Score</th><th className="text-right">Evidence state</th></tr></thead><tbody>{filteredPassports.map((passport) => <tr key={passport.id}><td><span className="block text-xs font-extrabold text-[#d4d4d4]">{passport.name}</span><span className="font-mono text-[10px] text-[#6f6f6f]">v{passport.version} · {passport.publisher}</span></td><td><span className="rounded bg-[#2d2d2d] px-1.5 py-0.5 text-[9px] font-bold text-[#9d9d9d]">{passport.category}</span></td><td className="text-center font-mono font-bold text-[#d4d4d4]">{passport.overallScore ?? 'Not verified'}</td><td className="text-right text-[10px] font-mono text-[#6f6f6f]">{passport.overallScore == null ? 'Score unavailable' : 'Score observed'}</td></tr>)}{filteredPassports.length === 0 && <tr><td colSpan={4} className="py-6 text-center italic text-[#6f6f6f]">No matching passport records found.</td></tr>}</tbody></table></div></section>

      {isRemediationOpen && <Modal title="Remediation workflow" onClose={() => setIsRemediationOpen(false)}><p className="text-sm text-[#9d9d9d]">Start the protected backend remediation workflow for this tenant. The UI will only report what the backend returns.</p><button disabled={isRemediating} onClick={() => void triggerRemediation()} className="spr-btn spr-btn-primary mt-5 disabled:opacity-50">{isRemediating ? 'Running…' : 'Run remediation'}</button>{remediationCompleted && <p className="mt-4 text-xs text-[#89d185]">Remediation request completed; re-check the evidence source before treating any risk as resolved.</p>}{remediationLogs.length > 0 && <pre className="mt-4 max-h-52 overflow-auto spr-panel-alt p-3 text-xs text-[#d4d4d4]">{remediationLogs.join('\n')}</pre>}</Modal>}
      {isReportsOpen && <Modal title="Evidence report hub" onClose={() => setIsReportsOpen(false)}><p className="text-sm text-[#9d9d9d]">Reports compile current application records. Certification, external assurance, and attestation status remain unverified unless backed by a connected evidence source.</p><div className="mt-5 space-y-2"><button onClick={() => setSelectedReportId('executive')} className="spr-btn spr-btn-secondary w-full text-left">Executive evidence summary</button><button onClick={() => setSelectedReportId('procurement')} className="spr-btn spr-btn-secondary w-full text-left">Procurement evidence summary</button><button onClick={() => setSelectedReportId('compliance')} className="spr-btn spr-btn-secondary w-full text-left">Compliance evidence summary</button></div><p className="mt-4 text-xs text-[#9d9d9d]">Selected: {selectedReportId}. No certification seal is implied.</p></Modal>}
    </motion.div>
  </div>;
}

function Metric({ label, value, note, icon }: { label: string; value: number | null; note: string; icon: React.ReactNode }) { return <div className="relative flex flex-col justify-between overflow-hidden spr-panel p-5"><div className="flex items-center gap-2 text-[#6f6f6f]">{icon}<span className="text-[10px] font-mono font-extrabold uppercase tracking-widest">{label}</span></div><h4 className="mt-2 text-3xl font-black text-[#d4d4d4]">{value ?? 'Not verified'}</h4><p className="mt-1 text-[10px] font-mono leading-none text-[#6f6f6f]">{note}</p></div>; }
function ActionCard({ title, description, button, icon, onClick }: { title: string; description: string; button: string; icon: React.ReactNode; onClick: () => void }) { return <div className="flex flex-col justify-between spr-panel p-5"><div><div className="flex h-9 w-9 items-center justify-center rounded-md border border-[#3c3c3c] bg-[#094771] text-[#3794ff]">{icon}</div><h4 className="mt-3 text-sm font-extrabold text-[#d4d4d4]">{title}</h4><p className="mt-2 text-[11px] leading-relaxed text-[#9d9d9d]">{description}</p></div><button onClick={onClick} className="spr-btn spr-btn-primary mt-4 flex items-center justify-center gap-1.5">{button}<ArrowRight className="h-3.5 w-3.5"/></button></div>; }
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-5" role="dialog" aria-modal="true"><div className="w-full max-w-xl spr-panel p-6"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold text-[#d4d4d4]">{title}</h2><button onClick={onClose} className="rounded-md border border-[#3c3c3c] px-2 py-1 text-[#9d9d9d]">×</button></div>{children}</div></div>; }
