  ...
  return <div className="space-y-8 py-4">
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-8 text-left">
      ...
      <div className="grid grid-cols-1 items-stretch gap-6 lg:grid-cols-12">
        <div className="relative flex min-h-[300px] flex-col justify-between overflow-hidden spr-panel p-6 text-[var(--spr-text)] lg:col-span-5"><div><span className="block text-[12px] font-semibold uppercase tracking-[.06em] text-[var(--spr-highlight)]">TRUST SCORE OBSERVATION</span><h3 className="text-sm font-black text-[var(--spr-text)]">Client Trust Index</h3></div><div className="flex items-center justify-around gap-6 py-2"><div className="relative flex h-36 w-36 items-center justify-center"><svg className="h-full w-full -rotate-90"><circle cx="72" cy="72" r="60" className="text-[var(--spr-border)]" strokeWidth="8" stroke="currentColor" fill="transparent"/><circle cx="72" cy="72" r="60" className={trustHealth === null ? 'text-[var(--spr-text-faint)]' : 'text-[var(--spr-highlight)]'} strokeWidth="8" strokeDasharray={377} strokeDashoffset={trustHealth === null ? 377 : 377 - (377 * trustHealth) / 100} strokeLinecap="round" stroke="currentColor" fill="transparent"/></svg><div className="absolute flex flex-col items-center justify-center text-center"><span className="text-3xl font-black">{trustHealth === null ? '—' : trustHealth}</span><span className="text-[11px] font-bold uppercase tracking-wider text-[var(--spr-text-muted)]">Observed score</span></div></div><div className="space-y-2 font-mono text-[12px] text-[var(--spr-text-muted)]"><div className="flex justify-between gap-4 border-b border-[var(--spr-border)] pb-1"><span>SCORE STATUS:</span><strong className="text-[var(--spr-text)]">{trustHealth === null ? 'NOT VERIFIED' : 'OBSERVED'}</strong></div><p className="pt-1 text-[11px] leading-normal text-[var(--spr-text-faint)]">{trustHealth === null ? 'No client trust scores were returned by the backend.' : 'Average of returned client trust scores.'}</p></div></div><div className="border-t border-[var(--spr-border)] pt-3 text-[11px] font-mono text-[var(--spr-text-faint)]">External assurance, certifications, and infrastructure claims require separately connected evidence.</div></div>
