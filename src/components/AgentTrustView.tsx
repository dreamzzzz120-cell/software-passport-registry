...
const agents = [
  ...
  { name: 'Monitoring Agent', status: 'LIVE', description: 'Detects deterministic material changes, resolution, staleness and no-op observations from SPR evidence.', action: 'Run monitoring review via API', icon: Activity },
  { name: 'Report Agent', status: 'LIVE', description: 'Turns verified evidence, findings, freshness and agent results into provenance-aware customer-ready reports.', action: 'Generate report via API', icon: Copy },
  { name: 'Revenue Agent', status: 'LIVE', description: 'Identifies evidence-grounded MSP service opportunities without changing trust, pricing, findings or compliance.', action: 'Run revenue review via API', icon: Sparkles },
] as const;
...
export default function AgentTrustView() {
  ...
  return <div className="mx-auto max-w-7xl space-y-8">
    ...
    <section>
      <div className="mb-4 flex items-end justify-between gap-4"><div><h2 className="text-xl font-semibold">Agent workforce</h2><p className="mt-1 text-sm text-[var(--spr-text-muted)]">One SPR evidence layer. Specialized workers on top.</p></div><div className="text-xs text-[var(--spr-text-faint)]">Live means backed by current repository functionality.</div></div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {agents.map(({ name, status, description, action, icon: Icon }) => <article key={name} className="spr-panel p-5">
          <div className="flex items-start justify-between gap-3"><div className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-2"><Icon className="h-5 w-5" /></div><span className="rounded-full bg-[var(--spr-green)]/15 px-2.5 py-1 text-[10px] font-bold tracking-[.16em] text-[var(--spr-green)]">{status}</span></div>
          ...
          <div className="mt-5 flex items-center gap-2 text-xs font-semibold text-[var(--spr-text-faint)]"><CheckCircle2 className="h-4 w-4 text-[var(--spr-green)]" />{action}</div>
        </article>)}
      </div>
    </section>
    ...
    <section className="spr-panel p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-semibold">Agent tools</h2><p className="mt-1 text-sm text-[var(--spr-text-muted)]">External agents can use these read-only trust capabilities through MCP. Autonomous specialist agents are available through the authenticated Agent API.</p></div><button onClick={() => void copy(endpoint)} className="spr-btn spr-btn-secondary inline-flex items-center gap-2"><Copy className="h-4 w-4" />{copied ? 'Copied' : 'Copy MCP endpoint'}</button></div><div className="mt-4 grid gap-3 md:grid-cols-2">{tools.map(([name, description]) => <div key={name} className="rounded-md border border-[var(--spr-border)] p-4"><div className="font-mono text-sm text-[var(--spr-highlight)]">{name}</div><p className="mt-1 text-sm text-[var(--spr-text-muted)]">{description}</p></div>)}</div></section>
