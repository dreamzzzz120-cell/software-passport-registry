...
export default function AgentTrustView() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <div className="spr-panel p-5"><div className="text-sm text-[var(--spr-text)]">Trust rule</div><div className="mt-2 text-xl font-semibold">Evidence first</div><div className="mt-1 text-xs text-[var(--spr-text-muted)]">Missing evidence stays unverified.</div></div>
    </div>
    <section className="spr-panel p-5"><h2 className="text-lg font-semibold">Available agent tools</h2><div className="mt-4 grid gap-3 md:grid-cols-2">{tools.map(([name, description]) => <div key={name} className="rounded-md border border-[var(--spr-border)] p-4"><div className="font-mono text-sm text-[var(--spr-highlight)]">{name}</div><p className="mt-1 text-sm text-[var(--spr-text-muted)]">{description}</p></div>)}</div></section>
    <section className="grid gap-5 lg:grid-cols-2">
      <div className="spr-panel p-5"><h2 className="text-lg font-semibold">Verify a Passport</h2><p className="mt-1 text-sm text-[var(--spr-text-muted)]">Paste a signed public Passport reference to prepare an agent verification request.</p><input value={passport} onChange={e => setPassport(e.target.value.slice(0, 512))} placeholder="Signed Passport token or URL" className="mt-4 w-full rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-sm outline-none focus:border-[var(--spr-highlight)]" /><pre className="mt-4 max-h-64 overflow-auto rounded-md bg-[var(--spr-surface-sunken)] p-4 text-xs text-[var(--spr-text)]">{example}</pre><button onClick={() => void copy(example)} className="spr-btn spr-btn-primary mt-3">{copied ? 'Copied' : 'Copy JSON-RPC request'}</button></div>
      <div className="spr-panel p-5"><h2 className="text-lg font-semibold">Verify a claim</h2><p className="mt-1 text-sm text-[var(--spr-text-muted)]">SPR deliberately returns UNVERIFIED when observed evidence does not support the claim.</p><input value={claim} onChange={e => setClaim(e.target.value.slice(0, 2000))} placeholder="Example: this software has current security evidence" className="mt-4 w-full rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-sm outline-none focus:border-[var(--spr-highlight)]" /><div className="mt-4 rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-4 text-sm text-[var(--spr-amber)]">Claim: {claim || 'Enter a claim to send to an MCP-compatible agent.'}</div></div>
    </section>
  </div>;
}
