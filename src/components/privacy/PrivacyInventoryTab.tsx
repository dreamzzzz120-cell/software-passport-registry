        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-6 shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="mt-5 space-y-3">
              ...
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Information type * (e.g. Client contact details)<input required value={form.informationType} onChange={(e) => setForm((c) => ({ ...c, informationType: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Category<input value={form.category} onChange={(e) => setForm((c) => ({ ...c, category: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Purpose<textarea value={form.purpose} onChange={(e) => setForm((c) => ({ ...c, purpose: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Retention<input value={form.retention} onChange={(e) => setForm((c) => ({ ...c, retention: e.target.value }))} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
