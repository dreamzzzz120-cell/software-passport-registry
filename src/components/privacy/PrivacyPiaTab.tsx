        ...
        <div className="max-h-[560px] divide-y divide-[var(--spr-border)] overflow-y-auto">
          {loading ? <div className="p-6 text-sm text-[var(--spr-text-muted)]">Loading…</div> : pias.length === 0 ? (
            ...
            <button key={p.id} onClick={() => setSelectedId(p.id)} className={`flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm transition ${selectedId === p.id ? 'bg-[var(--spr-accent-soft)]/45' : 'hover:bg-[var(--spr-surface-sunken)]'}`}>
              ...
              <span className={`shrink-0 rounded-sm border px-2 py-0.5 text-[12px] font-bold uppercase ${DECISION_STYLE[p.decision]}`}>{p.decision.replace('_', ' ')}</span>
            </button>
          ))}
        </div>
      </section>
            ...
            {selected.decision === 'PENDING' ? (
              canWrite && (
                <div className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-deep)] p-3">
                  <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Reviewer name * (required to record a decision)<input value={reviewerName} onChange={(e) => setReviewerName(e.target.value)} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-2.5 py-1.5 text-xs text-[var(--spr-text)]" /></label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-6 shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="mt-5 space-y-3">
              ...
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Processing description *<textarea required value={form.processingDescription} onChange={(e) => setForm((c) => ({ ...c, processingDescription: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Personal information involved<textarea value={form.personalInformationDescription} onChange={(e) => setForm((c) => ({ ...c, personalInformationDescription: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Risks<textarea value={form.risks} onChange={(e) => setForm((c) => ({ ...c, risks: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
              <label className="flex flex-col gap-1 text-[12px] font-bold text-[var(--spr-text-muted)]">Safeguards<textarea value={form.safeguards} onChange={(e) => setForm((c) => ({ ...c, safeguards: e.target.value }))} rows={2} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] px-3 py-2 text-xs text-[var(--spr-text)]" /></label>
