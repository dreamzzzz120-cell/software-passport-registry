...
export default function NewReviewIntake() {
  ...
  const addFiles = (list: FileList | File[]) => {
    ...
    setFiles((current) => [...current, ...incoming.filter((f) => !current.some((x) => x.name === f.name && x.size === f.size && x.lastModified === f.lastModified))].slice(0, 100));
    setError(''); setMessage('');
  };
  ...
  const startRepository = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (url.hostname.toLowerCase() !== 'github.com') throw new Error('Enter a GitHub repository URL or owner/repository.');
      const parts = url.pathname.split('/').filter(Boolean).map((part) => part.replace(/\.git$/, ''));
      if (parts.length !== 2) throw new Error('Use owner/repository, for example acme/my-app.');
      ...
      if (typeof body?.statusUrl === 'string') setMessage(`Repository review started for ${parts[0]}/${parts[1]}. Status: ${body.statusUrl}`);
  ...
  return <section className="spr-panel p-5 md:p-6" aria-label="Start a new software review">
    <div className="flex flex-col gap-1">
      <div className="text-[10px] font-bold uppercase tracking-[.2em] text-[var(--spr-highlight)]">New review</div>
      <h2 className="text-xl font-bold text-[var(--spr-text)]">Submit software for analysis</h2>
      <p className="max-w-3xl text-sm text-[var(--spr-text-muted)]">Choose a GitHub repository or upload software and evidence files. Uploads are finalized with SHA-256 before they enter the analysis queue.</p>
    </div>
