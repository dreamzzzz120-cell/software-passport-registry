...
export default function MfaSettingsPanel({ currentUser }: { currentUser: User | null }) {
  ...
  return <div className="spr-panel p-5 space-y-4">
    <div className="flex items-center justify-between border-b border-[var(--spr-border)] pb-2">
      ...
      <span className={`text-[12px] font-mono font-bold ${enrolled ? 'text-[var(--spr-green)]' : 'text-[var(--spr-amber)]'}`}>{enrolled ? 'TOTP ENROLLED' : 'NOT ENROLLED'}</span>
    </div>
    <p className="text-[12px] text-[var(--spr-text-muted)] leading-relaxed">This control uses Firebase Authentication's real TOTP factor. SPR does not store your authenticator secret.</p>
