...
const authMessage = (error: any, fallback: string) => {
  switch (error?.code) {
    ...
    case 'auth/unauthorized-domain': return `This site is not authorized for Firebase sign-in (${window.location.hostname}).`;
...
export default function LoginView({ onLoginSuccess }: LoginViewProps) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const message = authMessage(err, 'Sign-in failed.');
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#05070d] p-6 text-white">
      <form onSubmit={submit} className="w-full max-w-md space-y-5 rounded-3xl border border-white/[.08] bg-white/[.035] p-7 shadow-2xl backdrop-blur-2xl" noValidate>
        <div className="text-center">
          <div className="text-xs font-bold uppercase tracking-[.25em] text-cyan-300">Software Passport Registry</div>
          <h1 className="mt-3 text-3xl font-semibold">Sign in to SPR</h1>
          <p className="mt-2 text-sm text-slate-400">Use your work email or continue with Google.</p>
        </div>
        ...
        <label className="block text-sm font-semibold text-slate-200">Password
          <span className="relative mt-2 block">
            <input className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 pr-12 text-white outline-none focus:border-cyan-300/50" type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={6} required />
            <button type="button" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute inset-y-0 right-0 px-4 text-slate-400">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
          </span>
        </label>
