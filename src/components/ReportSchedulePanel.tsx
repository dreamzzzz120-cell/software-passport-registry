          ...
          <div key={schedule.id} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-sunken)] p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-sm text-[var(--spr-text)]">{passportName.get(schedule.passportId) || schedule.passportId}</span><span className="rounded-full border border-[var(--spr-border)] px-2 py-0.5 text-[12px] font-semibold uppercase tracking-wide text-[var(--spr-text-muted)]">{schedule.reportType}</span><span className={`rounded-full border px-2 py-0.5 text-[12px] font-semibold ${schedule.enabled ? 'border-[var(--spr-green)]/30 text-[var(--spr-green)]' : 'border-[var(--spr-border)] text-[var(--spr-text-muted)]'}`}>{schedule.enabled ? 'Active' : 'Paused'}</span></div>
