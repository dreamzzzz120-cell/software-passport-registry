    ...
    <section className="space-y-6" aria-labelledby="reports-title">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
        <div className="rounded-3xl border border-white/[.08] bg-white/[.035] p-6">
          ...
          {clients.length > 0 && (
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500">{exportClientIds.size === 0 ? 'All clients' : `${exportClientIds.size} of ${clients.length} clients selected`}</span>
              <div className="flex flex-wrap gap-1.5">
                {clients.map((client) => (
                  <button key={client.id} onClick={() => toggleExportClient(client.id)} className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${exportClientIds.has(client.id) ? 'border-cyan-300/30 bg-cyan-300/10 text-cyan-200' : 'border-white/10 bg-white/[.03] text-slate-500'}`}>{client.name}</button>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mt-6 border-t border-white/[.08] pt-5">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <select value={selectedPassportId} onChange={(event) => setSelectedPassportId(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-[#0b101b] px-3 py-2.5 text-sm text-slate-200">
              ...
              <select value={reportType} onChange={(event) => setReportType(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-[#0b101b] px-3 py-2.5 text-sm text-slate-200">
          ...
          <div className="mt-6 border-t border-white/[.08] pt-5">
            <div className="flex items-center gap-2"><History size={16} className="text-slate-400" /><h3 className="text-sm font-semibold text-slate-200">Report history</h3></div>
            <p className="mt-1 text-xs leading-5 text-slate-500">Every generated report is hashed and versioned. Load a passport report above to populate history for the selected type.</p>
            {historyLoading && <p className="mt-3 text-xs text-slate-500">Loading history…</p>}
            {!historyLoading && history.length === 0 && <p className="mt-3 text-xs text-slate-500">No prior snapshots for this passport and report type yet.</p>}
            {history.length > 0 && (
              <ul className="mt-3 max-h-48 space-y-2 overflow-auto pr-1">
                {history.map((snapshot) => (
                  <li key={snapshot.id}>
                    <button onClick={() => void loadSnapshot(snapshot.id)} className="w-full rounded-xl border border-white/[.07] bg-black/20 px-3 py-2 text-left text-xs text-slate-300 hover:border-cyan-300/25 hover:text-white">
                      <div className="flex items-center justify-between gap-2">
                        <span>{new Date(snapshot.generated_at).toLocaleString()}</span>
                        <span className="text-slate-500">score {snapshot.score}</span>
                      </div>
                      <div className="mt-1 truncate font-mono text-[10px] text-slate-500">{snapshot.canonical_payload_hash}</div>
                    </button>
                  </li>
                ))}
              </ul>
        ...
        <div className="rounded-3xl border border-white/[.08] bg-white/[.035] p-6">
          ...
          <p className="mt-2 text-sm text-slate-400">JSON is the exact server response. PDF is generated client-side from the loaded passport's SBOM, evidence, and timeline.</p>
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <button onClick={downloadPdf} disabled={!selectedPassport} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/25 bg-cyan-300/10 px-4 py-2.5 text-sm font-semibold text-cyan-100 disabled:opacity-40"><FileText size={16} /> Download PDF</button>
          </div>

          <div className="mt-6 border-t border-white/[.08] pt-5">
            <div className="flex items-center gap-2"><Share2 size={16} className="text-slate-400" /><h3 className="text-sm font-semibold text-slate-200">Shareable link</h3></div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <button onClick={() => void createShareLink()} disabled={!canShare || !selectedPassport || sharing} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-4 py-2.5 text-sm font-semibold text-slate-200 disabled:cursor-not-allowed disabled:opacity-40">
                <Share2 size={16} className={sharing ? 'animate-pulse' : ''} /> {sharing ? 'Creating…' : 'Create share link'}
              </button>
              {shareInfo && (
                <button onClick={() => void copyShareLink()} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/25 bg-cyan-300/10 px-3 py-2.5 text-xs font-semibold text-cyan-100"><Copy size={14} /> Copy link</button>
              )}
            </div>
            {shareInfo && (
              <div className="mt-3 rounded-xl border border-white/[.07] bg-black/20 p-3 text-xs text-slate-400">
                <div className="truncate font-mono text-slate-300">{shareInfo.shareUrl}</div>
                <div className="mt-1 text-slate-500">Expires {new Date(shareInfo.expiresAt).toLocaleString()}</div>
              </div>
            )}
            {shareMessage && <p className="mt-2 text-xs text-slate-400" role="status">{shareMessage}</p>}
          </div>

          <pre className="mt-6 max-h-64 overflow-auto rounded-2xl border border-white/[.07] bg-black/20 p-4 text-xs leading-5 text-slate-400 whitespace-pre-wrap">{reportText || 'Load a passport report to preview its persisted traceability and limitations.'}</pre>
        </div>
      </div>

      {report?.sbom && (
        <div className="rounded-3xl border border-white/[.08] bg-white/[.035] p-6">
          <div className="flex items-center gap-2"><Package size={18} className="text-cyan-200" /><h2 className="text-lg font-semibold">SBOM components</h2></div>
          <p className="mt-2 text-sm text-slate-400">Each component is cross-referenced against this passport's recorded vulnerabilities by name.</p>
          <div className="mt-4 overflow-auto">
            <table className="w-full min-w-[560px] text-left text-xs">
              <thead className="text-slate-500"><tr><th className="pb-2 pr-4">Component</th><th className="pb-2 pr-4">Version</th><th className="pb-2 pr-4">License</th><th className="pb-2 pr-4">Vulnerabilities</th><th className="pb-2">Critical/High</th></tr></thead>
              <tbody className="text-slate-300">
                {report.sbom.length === 0 && <tr><td colSpan={5} className="py-3 text-slate-500">No SBOM components recorded for this passport.</td></tr>}
                {report.sbom.map((component, index) => (
                  <tr key={`${component.name}-${index}`} className="border-t border-white/[.05]">
      ...
      {report?.controls && (
        <div className="rounded-3xl border border-white/[.08] bg-white/[.035] p-6">
          <div className="flex items-center gap-2"><ShieldCheck size={18} className="text-cyan-200" /><h2 className="text-lg font-semibold">Findings by control</h2></div>
          <p className="mt-2 text-sm text-slate-400">Open trust findings grouped by control ID, worst-observed severity first.</p>
          <div className="mt-4 overflow-auto">
            <table className="w-full min-w-[480px] text-left text-xs">
              <thead className="text-slate-500"><tr><th className="pb-2 pr-4">Control</th><th className="pb-2 pr-4">Findings</th><th className="pb-2 pr-4">Open</th><th className="pb-2">Worst severity</th></tr></thead>
              <tbody className="text-slate-300">
                {report.controls.length === 0 && <tr><td colSpan={4} className="py-3 text-slate-500">No control-mapped findings for this passport.</td></tr>}
                {report.controls.map((control) => (
                  <tr key={control.controlId} className="border-t border-white/[.05]">
      ...
      {clients.length > 0 && (
        <div className="rounded-3xl border border-white/[.08] bg-white/[.035] p-6">
          <h2 className="text-lg font-semibold text-white">White-label client report</h2>
          <p className="mt-2 text-sm text-slate-400">Generates a co-branded PDF for one client using your MSP name, logo, and brand color. Runs entirely in the browser from already-loaded client data.</p>
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              <select value={whiteLabelClientId} onChange={(event) => setWhiteLabelClientId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-[#0b101b] px-3 py-2.5 text-sm text-slate-200">
                {clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}
              </select>
              <input value={mspName} onChange={(event) => setMspName(event.target.value)} placeholder="Your MSP / firm name *" className="w-full rounded-xl border border-white/10 bg-[#0b101b] px-3 py-2.5 text-sm text-slate-200 placeholder:text-slate-600" />
              <input value={reportTitle} onChange={(event) => setReportTitle(event.target.value)} placeholder="Report title" className="w-full rounded-xl border border-white/10 bg-[#0b101b] px-3 py-2.5 text-sm text-slate-200" />
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 text-xs text-slate-400">Brand color <input type="color" value={brandColor} onChange={(event) => setBrandColor(event.target.value)} className="h-8 w-8 rounded border border-white/10 bg-transparent" /></label>
                <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-white/10 bg-white/[.03] px-3 py-2 text-xs text-slate-300 hover:border-cyan-300/25"><Upload size={14} /> {logoBase64 ? 'Logo uploaded' : 'Upload logo'}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={handleLogoUpload} className="hidden" /></label>
              </div>
              <textarea value={executiveSummary} onChange={(event) => setExecutiveSummary(event.target.value)} placeholder="Executive summary (optional)" rows={3} className="w-full rounded-xl border border-white/10 bg-[#0b101b] px-3 py-2.5 text-sm text-slate-200 placeholder:text-slate-600" />
            </div>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-xs text-slate-300">
                {(Object.keys(sections) as Array<keyof typeof sections>).map((key) => (
                  <label key={key} className="flex items-center gap-2 rounded-lg border border-white/[.07] bg-black/15 px-3 py-2 capitalize"><input type="checkbox" checked={sections[key]} onChange={(event) => setSections((current) => ({ ...current, [key]: event.target.checked }))} /> {key}</label>
                ))}
              </div>
              <button onClick={generateWhiteLabelReport} disabled={!whiteLabelClientId || !mspName.trim()} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-40"><FileText size={16} /> Generate white-label PDF</button>
              <p className="text-[11px] leading-5 text-slate-500">Uses the client's already-loaded software inventory and compliance status. No score is fabricated beyond what SPR already has on record for this client.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-amber-300/15 bg-amber-300/[.04] p-4 text-xs leading-5 text-amber-100/75">
        <strong className="text-amber-100">Capability boundary:</strong> PDF export runs entirely in the browser from already-loaded passport data; there is no server-rendered PDF. Share links are stateless signed URLs with a 7-day expiry and cannot be individually revoked before then. "Print / save PDF" still uses the browser's print dialog for a full-page copy. Empty datasets remain empty and are not replaced with sample values.
      </div>
    </section>
  );
}
