    ...
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3" ref={containerRef} id="quick-actions-speeddial-wrapper">
                ...
                <>
                  <div className="space-y-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" placeholder="e.g. redis-cache-primary" value={scanTarget} onChange={(e) => setScanTarget(e.target.value)} className="bg-slate-50 dark:bg-zinc-850 border border-slate-200 dark:border-zinc-750 text-xs rounded-xl p-3 focus:outline-none focus:border-indigo-500 focus:bg-white dark:text-zinc-200 font-semibold" />
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label className="font-bold text-slate-700 dark:text-zinc-300">Scan Strategy Type</label>
                        <select value={scanType} onChange={(e) => setScanType(e.target.value as any)} className="bg-slate-50 dark:bg-zinc-850 border border-slate-200 dark:border-zinc-750 text-xs rounded-xl p-3 focus:outline-none cursor-pointer text-slate-700 dark:text-zinc-300 font-semibold">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select value={scanClient} onChange={(e) => setScanClient(e.target.value)} className="bg-slate-50 dark:bg-zinc-850 border border-slate-200 dark:border-zinc-750 text-xs rounded-xl p-3 focus:outline-none cursor-pointer text-slate-700 dark:text-zinc-300 font-semibold">
                  ...
                  <div className="pt-4 border-t border-slate-100 dark:border-zinc-800 flex justify-end gap-2">
                    <button type="button" onClick={() => setActiveModal(null)} className="px-4 py-2.5 hover:bg-slate-100 dark:hover:bg-zinc-850 border border-slate-200 dark:border-zinc-800 rounded-xl text-slate-500 dark:text-zinc-400 font-semibold cursor-pointer">Cancel</button>
                    <button type="button" onClick={executeScan} disabled={!scanTarget.trim()} className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-extrabold rounded-xl shadow-md hover:shadow-indigo-500/10 transition cursor-pointer flex items-center gap-1.5"><Play className="w-3.5 h-3.5" /><span>Execute Scan Now</span></button>
                  </div>
                </>
              ) : isScanning ? (
                <div className="space-y-4 py-4 text-center">
                  <div className="relative w-20 h-20 mx-auto flex items-center justify-center"><Loader2 className="w-12 h-12 text-indigo-600 animate-spin absolute" /><Radar className="w-6 h-6 text-indigo-500 animate-pulse" /></div>
                  <div className="space-y-1"><span className="font-bold text-slate-800 dark:text-zinc-100">Scanning {scanTarget}...</span><div className="w-full max-w-xs mx-auto bg-slate-100 dark:bg-zinc-850 h-2 rounded-full overflow-hidden"><div className="bg-indigo-600 h-full rounded-full transition-all duration-100" style={{ width: `${scanProgress}%` }}></div></div><span className="text-[10px] font-mono text-slate-500 font-bold">{scanProgress}% completed</span></div>
                  <div className="bg-slate-950 text-slate-300 font-mono text-[10px] p-4 rounded-xl text-left h-36 overflow-y-auto border border-slate-800 space-y-1 select-text">{scanLogs.map((log, i) => <div key={i} className={`leading-relaxed ${log.startsWith('[SUCCESS]') ? 'text-emerald-400 font-bold' : ''}`}>{log}</div>)}</div>
                </div>
              ) : (
                <div className="text-center py-6 space-y-4">
                  <div className="w-12 h-12 bg-emerald-50 dark:bg-emerald-950/40 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400 mx-auto"><ShieldCheck className="w-7 h-7" /></div>
                  <div className="space-y-1"><h4 className="text-sm font-bold text-slate-800 dark:text-zinc-100">SBOM Attestation Created!</h4><p className="text-[10px] text-slate-500 dark:text-zinc-400">Target <span className="font-bold font-mono text-indigo-600 dark:text-indigo-400">{scanTarget}</span> has been securely cataloged and verified in the central trust ledger.</p></div>
                  <div className="pt-3 flex justify-center"><button onClick={() => { setActiveModal(null); resetScanWorkflow(); }} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl shadow-md transition cursor-pointer">Acknowledge & Close</button></div>
                </div>
              )}
            </div>
          </div>
      ...
      {activeModal === 'alert' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden flex flex-col">
            <div className="flex justify-between items-center bg-slate-50 dark:bg-zinc-950 px-6 py-4 border-b border-slate-150 dark:border-zinc-850">
              <div className="flex items-center gap-2.5"><div className="p-1.5 bg-rose-50 dark:bg-rose-950/40 rounded-lg text-rose-600 dark:text-rose-400"><AlertCircle className="w-5 h-5" /></div><div><h3 className="text-sm font-bold text-slate-900 dark:text-zinc-50 font-display">Log Security Alert</h3><p className="text-[10px] text-slate-500">Record an immediate software vulnerability or compliance gap</p></div></div>
              {!isLoggingAlert && <button onClick={() => setActiveModal(null)} className="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-zinc-200 cursor-pointer"><X className="w-4.5 h-4.5" /></button>}
            </div>
              ...
              {!alertCompleted ? (
                <form onSubmit={executeLogAlert} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="flex flex-col gap-1"><label className="font-bold text-slate-700 dark:text-zinc-300">Severity</label><select value={alertSeverity} onChange={(e) => setAlertSeverity(e.target.value as Severity)} className="bg-slate-50 dark:bg-zinc-850 border border-slate-200 dark:border-zinc-750 text-xs rounded-xl p-3 focus:outline-none cursor-pointer text-slate-700 dark:text-zinc-300 font-semibold"><option value="Critical">🚨 Critical</option><option value="High">🟠 High</option><option value="Medium">🟡 Medium</option><option value="Low">🔵 Low</option></select></div>
                    <div className="flex flex-col gap-1"><label className="font-bold text-slate-700 dark:text-zinc-300">Category</label><select value={alertCategory} onChange={(e) => setAlertCategory(e.target.value as any)} className="bg-slate-50 dark:bg-zinc-850 border border-slate-200 dark:border-zinc-750 text-xs rounded-xl p-3 focus:outline-none cursor-pointer text-slate-700 dark:text-zinc-300 font-semibold"><option value="Vulnerability">Vulnerability</option><option value="Compliance Gap">Compliance Gap</option><option value="Unverified Attestation">Unverified Attestation</option><option value="Policy Violation">Policy Violation</option></select></div>
                    <div className="flex flex-col gap-1"><label className="font-bold text-slate-700 dark:text-zinc-300">Client Tenant</label><select value={alertClient} onChange={(e) => setAlertClient(e.target.value)} className="bg-slate-50 dark:bg-zinc-850 border border-slate-200 dark:border-zinc-800 text-xs rounded-xl p-3 focus:outline-none cursor-pointer text-slate-700 dark:text-zinc-300 font-semibold">{clients.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></div>
                  </div>
