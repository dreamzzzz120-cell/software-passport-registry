  ...
  async function submit(raw?: string) {
    ...
    try {
      ...
      if (payload.action?.type === 'verify' && payload.action.endpoint && payload.action.payload) {
        ...
        const textResult = verify.status === 404 ? 'That software is UNKNOWN because SPR has no matching registered software. No negative trust claim was made.' : verify.ok ? `Evidence-backed result: ${result.status || 'UNKNOWN'}.` : 'Verification could not be completed. No trust claim was made.';
