...
type AgentResult = { status?: string; evidence?: { count?: number; completeness?: number | null; latestObservationAt?: string | null }; findings?: { open?: number; criticalOrHigh?: number } };
type CommandResponse = { reply?: string; path?: string; action?: { type?: string; endpoint?: string; payload?: Record<string, unknown> }; data?: { counts?: Record<string, number>; topPassports?: Array<{ passport_id: string; name: string; open_findings: number; critical_high: number }> }; actions?: Array<{ label: string; command?: string; path?: string }> };
type Message = { role: 'user' | 'agent'; text: string; result?: AgentResult; data?: CommandResponse['data']; actions?: CommandResponse['actions'] };
...
export default function ExperienceAgent() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: 'agent', text: 'I’m your SPR Agent. Tell me what you need. I can summarize observed risk, inspect software, and take you to the right workspace.' }]);
  ...
  async function submit(raw?: string) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: 'agent', text: payload.reply || 'Done.', data: payload.data, actions: payload.actions }]);
      ...
      if (payload.action?.type === 'verify' && payload.action.endpoint && payload.action.payload) {
        ...
        const textResult = verify.status === 404 ? 'That software is UNKNOWN because SPR has no matching registered software. No negative trust claim was made.' : verify.ok ? `Observed evidence result: ${result.status || 'UNKNOWN'}.` : 'Verification could not be completed. No trust claim was made.';
        setMessages((current) => [...current, { role: 'agent', text: textResult, result }]);
      }
    } catch (error) {
      console.error('[SPR Agent] command failed', error);
      setMessages((current) => [...current, { role: 'agent', text: error instanceof Error ? error.message : 'The agent service could not be reached. No trust claim was made.' }]);
