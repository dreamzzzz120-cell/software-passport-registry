  ...
  return <section className="space-y-5" id="extension-marketplace-view">
    <header className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="text-[12px] font-bold uppercase tracking-[.2em] text-[#c586c0]">SPR capability system</div>
      ...
      {filtered.map((extension) => {
        ...
        return <article key={extension.id} className="rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface-alt)] p-4 transition-colors hover:border-[var(--spr-border-strong,var(--spr-border))]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-1.5"><span className="text-[11px] font-bold uppercase tracking-[.16em] text-[var(--spr-text-faint)]">{categoryFor(extension)}</span><span className="rounded-full border border-[var(--spr-border)] px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-[.08em] text-[var(--spr-text-faint)]">{scopeFor(extension)}</span></div><h2 className="mt-1 font-semibold">{extension.name}</h2></div>
            {installedNow && <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-[var(--spr-green)]/40 bg-[var(--spr-green)]/15 px-2 py-1 text-[12px] font-semibold text-[var(--spr-green)]"><CheckCircle2 size={12} /> Installed</span>}
          </div>
          <p className="mt-2.5 text-sm leading-5 text-[var(--spr-text-muted)]">{extension.description}</p>
          <div className="mt-3 flex flex-wrap gap-1.5">{extension.steps.map((step) => <span key={step} className="rounded-full border border-[var(--spr-border)] px-2 py-1 text-[11px] text-[var(--spr-text-muted)]">{step}</span>)}</div>
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-5" role="dialog" aria-modal="true" aria-label={`${selected.name} details`}>
      <div className="w-full max-w-2xl rounded-md border border-[var(--spr-border)] bg-[var(--spr-surface)] p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4"><div><div className="text-[12px] font-bold uppercase tracking-[.2em] text-[var(--spr-highlight)]">{categoryFor(selected)} capability · {scopeFor(selected)}</div><h2 className="mt-2 text-xl font-semibold">{selected.name}</h2></div><button onClick={() => setSelected(null)} aria-label="Close capability details" className="rounded-lg border border-[var(--spr-border)] p-2 text-[var(--spr-text-muted)]">×</button></div>
        <p className="mt-3 text-sm leading-6 text-[var(--spr-text-muted)]">{selected.description}</p>
        <div className="mt-4 grid gap-4 md:grid-cols-2"><div><div className="text-[12px] uppercase tracking-[.16em] text-[var(--spr-text-faint)]">Workflow</div><div className="mt-2 space-y-2">{selected.steps.map((step, index) => <div key={step} className="flex items-center gap-2 rounded-xl border border-[var(--spr-border)] p-2.5 text-sm"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[var(--spr-surface-alt)] text-xs">{index + 1}</span>{step}</div>)}</div></div><div><div className="text-[12px] uppercase tracking-[.16em] text-[var(--spr-text-faint)]">Owning surfaces</div><div className="mt-2 space-y-2">{selected.sourceRoutes.map((route) => <button key={route} onClick={() => onNavigateTab?.(route)} className="flex w-full items-center justify-between rounded-xl border border-[var(--spr-border)] p-2.5 text-left text-sm hover:bg-[var(--spr-surface-alt)]">{route}<ChevronRight size={14} /></button>)}</div></div></div>
