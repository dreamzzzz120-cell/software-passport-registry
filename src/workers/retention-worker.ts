...
export async function runRetentionWorkerLoop(): Promise<void> {
  ...
  try {
    ...
    await pool.query(`UPDATE object_files o SET status='DELETED', deleted_at=CURRENT_TIMESTAMP WHERE o.status='ACTIVE' AND o.created_at < CURRENT_TIMESTAMP - (SELECT r.evidence_days || ' days' FROM retention_policies r WHERE r.tenant_id=o.tenant_id)`);
