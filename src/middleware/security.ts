...
export class RedisStore implements RateLimitStore {
  ...
  async incr(key: string, windowMs: number, limit: number): Promise<RateLimitRecord> {
    const result = await this.atomicClient.increment(this.lua, key, windowMs, limit);
...
export function setRateLimiterStore(store: RateLimitStore) {
...
export const rateLimiter = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    console.error(`[RateLimiter] fail-closed (requestId=${requestId}):`, err instanceof Error ? err.message : String(err));
...
export const requireAuth = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const exempt = ['/api/user/me', '/api/auth/resend-verification', '/api/auth/verify-status'].includes(req.path);
    if (!emailVerified && !exempt) return res.status(403).json({ error: 'Email verification required', code: 'EMAIL_NOT_VERIFIED' });
