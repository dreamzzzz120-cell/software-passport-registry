...
describe('Supabase authentication contract', () => {
  ...
  it('does not require email confirmation before workspace access', () => { const login = read('src/components/LoginView.tsx'); expect(login).not.toContain('Check your email and confirm your account before signing in.'); expect(login).not.toContain('email_confirmed_at'); });
