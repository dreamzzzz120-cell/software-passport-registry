...
const failedBranch = () => {
  const start = view.indexOf("result.scanStatus === 'failed' ? (");
  const end = view.indexOf('\n                </section>', start);
  return start >= 0 && end > start ? view.slice(start, end) : '';
};
...
describe('a scan where nothing ran is never presented as a clean result', () => {
  ...
  it('the UI never shows a success tick or count tiles for a failed scan', () => {
    ...
    expect(branch).toMatch(/We couldn’t scan this repository/i);
    expect(branch).toContain('role="alert"');
    expect(branch).not.toContain('CheckCircle2');
    expect(branch).not.toContain('Open findings');
    expect(branch).not.toContain('Evidence items');
    expect(branch).not.toContain('claim this Passport');
    expect(branch).toMatch(/not a clean result|No evidence was collected/i);
  });
