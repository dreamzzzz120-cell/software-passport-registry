...
describe('SPR security release contracts', () => {
  ...
  it('enforces tenant boundaries in database migrations', () => expect(read('migrations/0004_tenant_resource_integrity.sql')).toContain('tenant_id'));
  ...
  it('keeps migration versions unique and explicitly accounts for legacy numbering', () => {
    ...
    expect(versions).toContain(64);
    expect(versions[versions.length - 1]).toBeGreaterThanOrEqual(64);
  });
  ...
  it('pins every GitHub Action to a commit, never to a movable tag', () => {
    const workflows = fs.readdirSync(path.join(root, '.github/workflows'))
      ...
      .map((name) => read(path.join('.github/workflows', name))).join('\n');
