...
// own "Create isolated emulator identities" step already creates these exact
// two users -- tenant-a@security.test and tenant-b@security.test -- using the
// emulator-assigned uid, which is the only way this emulator allows it. A
// second signUp for the same emails would additionally now collide with
// EMAIL_EXISTS even if the localId problem were fixed.
//
...
suite('Firebase emulator tenant identities', () => {
  it('creates isolated Tenant A and Tenant B identities', () => {
    ...
    // The whole point of the emailVerified fix upstream of this test: a token
    // that does not actually carry a verified email is not an isolated,
    // usable identity, it is a token requireAuth will reject outright.
