...
describe('authentication role boundary contract', () => {
  it('uses an explicit finite role allowlist', () => {
    expect(securitySource).toContain("const VALID_ROLES = new Set(['Owner', 'Admin', 'Technician', 'Viewer', 'Client']);");
    expect(securitySource).toContain('!VALID_ROLES.has(dbUser.role)');
  });

  it('fails closed for malformed Client principals', () => {
    expect(securitySource).toContain("dbUser.role === 'Client' && !dbUser.clientId");
  });
