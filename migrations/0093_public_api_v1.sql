...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'distribution_contacts_pipeline_stage_check') THEN
...
CREATE INDEX IF NOT EXISTS distribution_contacts_pipeline_stage_idx ON distribution_contacts (tenant_id, pipeline_stage, updated_at DESC);
...
INSERT INTO distribution_campaign_settings (tenant_id) VALUES ('tenant-free-review-system') ON CONFLICT (tenant_id) DO NOTHING;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='distribution_campaign_settings' AND policyname='spr_tenant_isolation') THEN
    CREATE POLICY spr_tenant_isolation ON distribution_campaign_settings
      USING (tenant_id = current_setting('app.tenant_id', true))
      WITH CHECK (tenant_id = current_setting('app.tenant_id', true));
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='spr_app_runtime') THEN GRANT SELECT, INSERT, UPDATE ON distribution_campaign_settings TO spr_app_runtime; END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='spr_worker_runtime') THEN GRANT SELECT ON distribution_campaign_settings TO spr_worker_runtime; END IF;
END $$;
