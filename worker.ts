...
async function main(){await verifyDatabase();ready=true;const scanConsumers=Math.max(1,Math.min(4,Number.parseInt(process.env.SCAN_QUEUE_CONSUMERS??'2',10)||2));const scanSupervisors=Array.from({length:scanConsumers},(_,index)=>supervise(`osv-${index+1}`,runWorkerLoop));await Promise.all([...scanSupervisors,supervise('security',runSecurityScannerLoop),supervise('trust-monitoring',runTrustMonitoringWorkerLoop),supervise('webhook',runWebhookWorkerLoop),supervise('notifications',runNotificationWorkerLoop),supervise('retention',runRetentionWorkerLoop),supervise('report-schedules',runReportScheduleWorkerLoop)]);}
