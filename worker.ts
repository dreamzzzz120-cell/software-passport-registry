...
// Some loops run one cycle per invocation and return by design (report-schedules,
// notifications), so the start line is logged once per loop, not per cycle.
const startedLoops=new Set<string>();
async function supervise(name:string,run:()=>Promise<void>){let delay=1000;while(true){try{if(!startedLoops.has(name)){startedLoops.add(name);console.info(`[Worker] loop started: ${name}`);}await run();delay=1000;}catch(error){console.error(`[Worker] ${name} loop failure:`,error instanceof Error?error.message:String(error));if(config.sentry.dsn)Sentry.captureException(error, { tags: { worker_loop: name } });await new Promise(r=>setTimeout(r,delay));delay=Math.min(delay*2,30000);}}}
normalizeWorkerDatabaseEnv();startHealthServer();
async function main(){await verifyDatabase();ready=true;const scanConsumers=Math.max(1,Math.min(4,Number.parseInt(process.env.SCAN_QUEUE_CONSUMERS??'2',10)||2));const distributionEnabled=process.env.DISTRIBUTION_WORKER_ENABLED !== 'false';const distributionConsumers=Math.max(1,Math.min(20,Number.parseInt(process.env.DISTRIBUTION_WORKER_CONSUMERS??'2',10)||2));const scanSupervisors=[supervise('osv', runWorkerLoop),...Array.from({length:scanConsumers-1},(_,index)=>supervise(`osv-${index+2}`,runWorkerLoop))];const distributionSupervisors=distributionEnabled?Array.from({length:distributionConsumers},(_,index)=>supervise(`distribution-${index+1}`,runDistributionWorkerLoop)):[];await Promise.all([...scanSupervisors,...distributionSupervisors,supervise('security', runSecurityScannerLoop),supervise('trust-monitoring',runTrustMonitoringWorkerLoop),supervise('webhook',runWebhookWorkerLoop),supervise('notifications',runNotificationWorkerLoop),supervise('retention',runRetentionWorkerLoop),supervise('report-schedules',runReportScheduleWorkerLoop),supervise('registry-crawler',runPublicRepositoryAgentTeamLoop)]);}
